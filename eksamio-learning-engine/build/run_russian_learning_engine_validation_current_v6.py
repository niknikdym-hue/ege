#!/usr/bin/env python3
"""Current v6 aggregate local validation runner for Russian Learning Engine.

Uses hardened corrected source/practice builders plus runtime, selector, Skill
Graph and handoff checks. This is the current checkpoint gate.
"""
...
SUMMARY_REL = Path("audits/RUSSIAN-LEARNING-ENGINE-VALIDATION-CURRENT-V6-SUMMARY.txt")
...
def write_summary(root: Path, rows: list[dict[str, object]]) -> Path:
    ...
    lines = [
        ...
        "RUSSIAN LEARNING ENGINE — CURRENT V6 AGGREGATE VALIDATION",
        "",
    ...
    lines.extend([
        "",
        "HARDENED CORRECTION INVARIANTS",
        "- every disabled exception ID must exist before filtering",
        "- every disabled practice ID must exist before filtering",
        "- superseded `introductory_v_kontse_kontsov_dual_function` excluded",
        "- superseded `ex-practice-end-finally-001` excluded",
        "- replacement exception/practice available",

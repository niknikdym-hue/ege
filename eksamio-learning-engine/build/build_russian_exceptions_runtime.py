...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=root / "build" / "RUSSIAN-EXCEPTIONS-RUNTIME.json",
    )
    ...
    try:
        ...
        if not isinstance(exception_items, list):
            raise BuildError("Canonical Exceptions Bank missing items[]")
        if not isinstance(practice_items, list):
            raise BuildError("Canonical Practice Bank missing items[]")
        if not isinstance(priority_items, list):
            raise BuildError("Priority build missing items[]")
        ...
        for row in practice_items:
            if not isinstance(row, dict):
                raise BuildError("Non-object practice item")
            pid = row.get("practice_item_id")
            exception_id = row.get("exception_id")
            if not isinstance(pid, str) or not pid:
                raise BuildError("Practice item missing practice_item_id")
            if pid in seen_practice:
                raise BuildError(f"Duplicate practice_item_id: {pid}")
            seen_practice.add(pid)
            if not isinstance(exception_id, str) or not exception_id:
                raise BuildError(f"{pid}: missing exception_id")
            if row.get("status") not in {"source_verified", "reviewed"}:
                continue
            ...
            compact = {
                ...
                "feedback": compact_feedback(row.get("feedback")),
                "context_signature": row.get("context_signature"),
                "transfer_level": row.get("transfer_level"),
                "status": "enabled",
            }
            if "alt_answers" in row:
                compact["alt_answers"] = row["alt_answers"]
            if "distractors" in row:
                compact["distractors"] = row["distractors"]
        ...
        for row in exception_items:
            ...
            exception_id = row.get("exception_id")
            if not isinstance(exception_id, str) or not exception_id:
                raise BuildError("Exception missing exception_id")
            if exception_id in seen_exception:
            ...
            if not practice_ids:
                # Keep source bank larger than public runtime; no practice means not enabled yet.
                continue
            ...
            if priority is None:
                raise BuildError(f"{exception_id}: missing launch priority row")
        ...
        # Every enabled practice must resolve to an enabled runtime exception.
        orphan_practice = sorted(
            pid
            for pid, row in runtime_practice.items()
            if row["exception_id"] not in runtime_exceptions
        )
        if orphan_practice:
            raise BuildError(
                f"Enabled practice items without runtime exception: {orphan_practice[:10]}"
            )

        payload = {
            "schema_version":"1.0.0",
            "product_id":"russian_exceptions",
            "content_version":datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"),
            "generated_at":datetime.now(timezone.utc).isoformat(),
            "topics":sorted(topics.values(), key=lambda row:(row["order"], row["topic_id"])),
        ...
        print(
            f"PASS: runtime exceptions={len(runtime_exceptions)}, practice={len(runtime_practice)}, topics={len(topics)}"
        )

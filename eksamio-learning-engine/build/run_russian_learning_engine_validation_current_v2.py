#!/usr/bin/env python3
"""Current v2 aggregate local validation runner for Russian Learning Engine.

Includes canonical data builders, session selector tests and exact handoff-map
cross-link validation. No network, Git, Tilda, or production mutation.
"""
...
SUMMARY_REL = Path("audits/RUSSIAN-LEARNING-ENGINE-VALIDATION-CURRENT-V2-SUMMARY.txt")
...
def write_summary(root: Path, rows: list[dict[str, object]]) -> Path:
    ...
    lines = [
        ...
        "RUSSIAN LEARNING ENGINE — CURRENT V2 AGGREGATE VALIDATION",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "SAFETY",
            "- Data/tests only; no production mutation.",
            "- Current EGE trainer remains frozen/unchanged.",
            "- PASS does not authorize Tilda publication or main-trainer handoff.",
            "",
        ]
    )

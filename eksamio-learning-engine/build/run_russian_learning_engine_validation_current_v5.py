#!/usr/bin/env python3
"""Current v5 aggregate local validation runner for Russian Learning Engine.

Builds current corrected canonical data, derived launch priority, minimal browser
runtime, selector tests, independent Skill Graph audit and exact handoff checks.
...
SUMMARY_REL = Path("audits/RUSSIAN-LEARNING-ENGINE-VALIDATION-CURRENT-V5-SUMMARY.txt")
...
def write_summary(root: Path, rows: list[dict[str, object]]) -> Path:
    ...
    lines = [
        ...
        "RUSSIAN LEARNING ENGINE — CURRENT V5 AGGREGATE VALIDATION",
        "",
    ...
    lines.extend([
        ...
        "- corrected replacement exception/practice included when valid",
        "- active practice total remains 72",
        "",
        "SAFETY",
        "- Current EGE trainer remains frozen/unchanged.",
        "- No Tilda/public write is performed.",
        "- PASS is a source/data/runtime-build checkpoint, not publication authorization.",
        "",
    ])

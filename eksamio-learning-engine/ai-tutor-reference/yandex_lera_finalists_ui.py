...
from yandex_speechkit_v3_tts import UrllibStreamingJsonTransport, YandexSpeechKitV3TTS, _split_text
...
class App:
    ...
    def synthesize(self, finalist: str) -> dict[str, object]:
        ...
        chunks = _split_text(spoken, MAX_CHUNK_CHARS)
        if not chunks:
            raise ValueError("finalist speech produced no chunks")
        if any(len(chunk) > MAX_CHUNK_CHARS for chunk in chunks):
            raise ValueError("finalist SpeechKit chunk exceeds configured limit")
        ...
        for chunk in chunks:
            ...
            if not found:
                raise RuntimeError("SpeechKit returned no audio for finalist chunk")
...
def main() -> int:
    ...
    app=App();Handler.app=app;server=ThreadingHTTPServer((HOST,PORT),Handler);url=f'http://{HOST}:{PORT}/';print(f'YANDEX_LERA_FINALISTS=READY {url}');print('FINALISTS=A,D');print('BRAIN_CALLS=0');print('PERSISTENT_AUDIO_BYTES=0');print(f'MAX_CHUNK_CHARS={MAX_CHUNK_CHARS}')

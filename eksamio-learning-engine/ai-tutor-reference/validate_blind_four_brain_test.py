...
def main() -> int:
    ...
    blind_text = command_paths["blind_text"].read_text(encoding="utf-8")
    blind_voice = command_paths["blind_voice"].read_text(encoding="utf-8")
    require("private_tutor_runtime_gate.zsh" in blind_text, "blind text launcher requires exact-build runtime gate")
    require("private_tutor_runtime_gate.zsh" in blind_voice, "blind voice launcher requires exact-build runtime gate")
    require("provider_live_preflight.py --require-four-brain" in blind_text, "blind text refuses to start until all four brains are locally ready")
    require("provider_live_preflight.py --require-four-brain --require-voice" in blind_voice, "blind voice refuses to start until all four brains and SpeechKit are locally ready")
    require(blind_text.index("--require-four-brain") < blind_text.index("Разрешаю слепой тест"), "blind text readiness is checked before paid-call authorization")
    require(blind_voice.index("--require-four-brain --require-voice") < blind_voice.index("Разрешаю слепой голосовой тест"), "blind voice readiness is checked before paid-call authorization")
    require("--enable-speech" not in blind_text, "blind text launcher cannot enable speech")
    require("--enable-speech" in blind_voice, "blind voice launcher explicitly enables the common speech layer")

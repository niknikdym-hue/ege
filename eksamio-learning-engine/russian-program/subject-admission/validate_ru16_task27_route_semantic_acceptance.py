...
def main() -> int:
    ...
    if tracked.get("canonical_school_registry_mutated") is not False or tracked.get("new_parallel_registry_created") is not False:
        raise AssertionError("RU16 acceptance must remain an overlay")
    expected_summary = {
    ...
    if tracked.get("summary") != expected_summary or generated.get("summary", {}).get("accepted_route_semantics") != 4:
        raise AssertionError("RU16 acceptance summary drift")

    tracked_decisions = {str(row.get("candidate_ref")): row for row in tracked.get("decisions", []) if isinstance(row, dict)}
    generated_decisions = {str(row.get("candidate_ref")): row for row in generated.get("decisions", []) if isinstance(row, dict)}
    ...
    for candidate_ref, (semantic_id, criterion_route) in EXPECTED.items():
        ...
        if generated_row.get("criterion_route") != expected_generated:
        ...
        if tracked_row.get("subject_semantic_status") != "CENTRAL_BRAIN_ACCEPTED_BOUNDED_ROUTE_SEMANTIC":
            raise AssertionError(f"RU16 semantic not explicitly accepted: {candidate_ref}")
    ...
    if authority.get("tier_a_fipi_spec_sha256") != "3b71ec81f954bc32b574a0b3b997ee37bb3bc19ae8825f11217fd7149198b476":
        raise AssertionError("RU16 Tier-A fingerprint drift")
    if authority.get("tier_a_locator") != "printed_page=20;pdf_page=10;panel=right;task=27":
        raise AssertionError("RU16 Task-27 locator drift")
    if set(authority.get("tier_a_content_codes") or []) != {"1.4", "1.5"}:
        raise AssertionError("RU16 Task-27 content scope drift")
    if set(authority.get("tier_a_checked_requirement_codes") or []) != {"1.5", "1.7"}:
        raise AssertionError("RU16 Task-27 requirement scope drift")
    if authority.get("tier_a_max_primary_score") != 22:
        raise AssertionError("RU16 Task-27 score drift")

    nonaccepted = {str(row.get("candidate_ref")): row for row in tracked.get("explicit_non_acceptances", []) if isinstance(row, dict)}
    ...
    k4_reason = str(nonaccepted["candidate-054"].get("reason", ""))
    if "criterion-proven" not in k4_reason or "source-verified EGE taxonomy node" not in k4_reason:
        raise AssertionError("candidate-054 K4 pending rationale drift")
    if nonaccepted["candidate-053"].get("criterion_route") != "K9_CONTRIBUTOR_ONLY":
        raise AssertionError("candidate-053 narrow K9 guard drift")

    c54 = [row for row in objects if row.get("object_key") == "semantic_candidate::candidate-054"]
    if len(c54) != 1:
        raise AssertionError("candidate-054 inventory object mismatch")
    c54 = c54[0]
    if c54.get("authority_status") != "current" or c54.get("audit_classification") != "MISSING_SUBJECT_SEMANTIC_CANDIDATE":
        raise AssertionError("candidate-054 authority/classification drift")
    if c54.get("current_semantic_refs") != ["essay_factual_accuracy"]:
        raise AssertionError("candidate-054 semantic ref drift")
    if "53-RUSSIAN-ESSAY-27-CRITERIA-MAP-2026.json#criteria[K4]" not in set(c54.get("evidence_provenance_refs") or []):
        raise AssertionError("candidate-054 K4 criterion provenance missing")
    if "candidate-054" in tracked_decisions:
        raise AssertionError("candidate-054 was silently admitted")
    ...
    if boundary.get("candidate_053_guard", {}).get("status") != "NARROW_GRAMMAR_CONTRIBUTOR_ONLY_NOT_K9_OWNER":
        raise AssertionError("RU16 candidate-053 boundary broadened")

    serialized = canonical_json(tracked).replace(b" ", b"")
    for forbidden in (b"ru-essay-language-correctness", b'"k4_k6_acceptances":1', b'"k7_k10_acceptances":1', b'"canonical_school_registry_mutated":true'):
    ...
    print("CANDIDATE_054_CRITERION_PROVEN_PENDING=1")

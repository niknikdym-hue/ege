#!/usr/bin/env python3
"""Current Sep-1 Russian launch progress with fourth exact RU01 phonetics object acceptance."""
...
BASE=H/"build_russian_semantic_acceptance_progress_launch_current_v6.py"
...
AUTH=H/"RU01-PHONETICS-P074-2-2-1-EXACT-CANONICAL-COMPONENT-ACCEPTANCE-v0.1.json"
CONTENT=H.parent/"production-learning-content"/"RU-PROG-01-PHONETICS-GRAPHICS-WAVE-001-v0.1.json"
BASE_HEAD="0216bd5395451c071a179a58364ef8fe2fd10203"
BASE_BLOB="04b3391d5eccbd7b0fa76c4d44798cca55011958"
BASE_SHA="b81407aa8498002234f92b12dd37ce0235e53077700fecbfe08e44feba15548d"
...
AUTH_SHA="c97613d165cd9107ad347e6fff6ed79295f214347d501d815f5d83bdcea00ced"
AUTH_ID="RU01_PHONETICS_P074_2_2_1_EXACT_CANONICAL_COMPONENT_ACCEPTANCE_v0.1"
UNIT="RAU-8e357e9bbe18a741725c"
REQ="RSK-EDSOO1011-2-2-1-P074"
GROUP="RUS-SEM-REVIEW-001"
SOURCE="EDSOO-RU-10-11-BASIC-2025"
DOC="EDSOO1011"
PAGE=74
CODE="2.2.1"
LOCATOR="EDSOO-RU-10-11-BASIC-2025/EDSOO1011 p.74 2.2.1"
...
def build_progress()->dict[str,Any]:
    if blob(BASE)!=BASE_BLOB:raise ValueError("current-v6 builder blob drift")
    d=runpy.run_path(str(BASE))["build_progress"]()
    if d.get("schema_version")!="0.9.0" or d.get("normalized_sha256")!=BASE_SHA:raise ValueError("current-v6 authority drift")
    s=d["progress_summary"]
    for k,v in EXPECTED_BASE.items():
        if s.get(k)!=v:raise ValueError(f"current-v6 aggregate drift: {k}")
    if len(d.get("accepted_authorities",[]))!=53:raise ValueError("current-v6 authority count drift")
    ...
    if UNIT not in ready.get("admission_unit_ids",[]) or REQ not in ready.get("requirement_ids",[]):raise ValueError("P074 2.2.1 no longer exact-reuse-ready")
    rec=[x for x in b.get("records",[]) if x.get("admission_unit_id")==UNIT and x.get("requirement_id")==REQ]
    if len(rec)!=1:raise ValueError("P074 2.2.1 binding not unique")
    r=rec[0]
    for k,v in {"source_id":SOURCE,"document_id":DOC,"page":PAGE,"code":CODE,"source_locator":LOCATOR,
        ...
        if r.get(k)!=v:raise ValueError(f"P074 2.2.1 binding identity drift: {k}")
    ...
    if a.get("status")!="CENTRAL_BRAIN_ACCEPTED_EXACT_RU01_PHONETICS_WORD_ANALYSIS_CANONICAL_COMPONENT_SET":raise ValueError("P074 2.2.1 authority status drift")
    if (a.get("current_launch_progress_v6_base_head_sha"),a.get("current_launch_progress_v6_builder_git_blob_sha1"),a.get("current_launch_progress_v6_normalized_sha256"))!=(BASE_HEAD,BASE_BLOB,BASE_SHA):raise ValueError("P074 2.2.1 base binding drift")
    if (a.get("exact_object_binding_review_sha256"),a.get("exact_source_review_input_sha256"))!=(BINDING_SHA,BINDING_INPUT_SHA):raise ValueError("P074 2.2.1 source binding drift")
    ...
    for k,v in exact.items():
        if dec.get(k)!=v:raise ValueError(f"P074 2.2.1 authority identity drift: {k}")
    m=dec.get("mastery_boundary") or {}
    if m.get("component_specific_independent_evidence_required") is not True or m.get("validated_word_analysis_item_may_support_only_its_single_canonical_ref") is not True:raise ValueError("P074 2.2.1 evidence boundary weakened")
    for k in ("generic_ru01_attempt_can_emit_exact_component_mastery","spelling_or_morphemic_evidence_can_substitute_for_word_analysis","shared_evidence_can_close_sibling_source_objects_without_separate_acceptance"):
        if m.get(k) is not False:raise ValueError(f"P074 2.2.1 mastery boundary weakened: {k}")
    if (a.get("policy") or {}).get("separate_object_acceptance_required") is not True or (a.get("policy") or {}).get("whole_group_acceptance_allowed") is not False:raise ValueError("P074 2.2.1 object-isolation guard weakened")
    if (a.get("summary") or {}).get("false_exact_mastery_admissions")!=0 or (a.get("summary") or {}).get("sibling_ru01_source_objects_accepted")!=0:raise ValueError("P074 2.2.1 authority boundary weakened")
    ...
    g=groups[0]
    if g.get("accepted_component_set_count")!=3:raise ValueError("current-v6 RU01 accepted-set count drift")
    ...
    if UNIT not in set(map(str,g.get("admission_unit_ids",[]))):raise ValueError("P074 2.2.1 unit missing from group")
    rows=[x for x in g.get("requirements",[]) if x.get("requirement_id")==REQ]
    if len(rows)!=1:raise ValueError("P074 2.2.1 requirement not unique")
    for k,v in (("source_id",SOURCE),("document_id",DOC),("page",PAGE),("code",CODE),("source_locator",LOCATOR)):
        if rows[0].get(k)!=v:raise ValueError(f"P074 2.2.1 source identity drift: {k}")
    accepted=[x for gg in d.get("semantic_review_groups",[]) for x in gg.get("accepted_component_sets",[])]
    if any(x.get("admission_unit_id")==UNIT or x.get("requirement_id")==REQ for x in accepted):raise ValueError("P074 2.2.1 already accepted")
    if any(x.get("id")==AUTH_ID for x in d.get("accepted_authorities",[])):raise ValueError("P074 2.2.1 authority already integrated")
    ...
    if g["accepted_component_set_count"]!=4:raise ValueError("P074 2.2.1 accepted-set count drift")
    ...
    for k,v in EXPECTED_AFTER.items():
        if s.get(k)!=v:raise ValueError(f"P074 2.2.1 post-acceptance aggregate drift: {k}")
    if len(d["accepted_authorities"])!=54:raise ValueError("P074 2.2.1 post-acceptance authority count drift")
    ...
    if len(new)!=1 or new[0].get("admission_unit_id")!=UNIT or new[0].get("admission_unit_id") in siblings:raise ValueError("P074 2.2.1 sibling leak")
    if s.get("false_exact_mastery_admissions")!=0:raise ValueError("false exact mastery must remain zero")

    d["schema_version"]="0.10.0"
    d["base_current_launch_progress_v6_head_sha"]=BASE_HEAD
    d["base_current_launch_progress_v6_builder_git_blob_sha1"]=BASE_BLOB
    d["base_current_launch_progress_v6_normalized_sha256"]=BASE_SHA
...
def main()->int:
    ...
    if a.emit:print(json.dumps(d,ensure_ascii=False,sort_keys=True,separators=(",",":")))
        ...
        s=d["progress_summary"];print("RUSSIAN_RU01_P074_2_2_1_EXACT_OBJECT_ACCEPTANCE=PASS");print("ACCEPTED_UNIT="+UNIT);print("ACCEPTED_REQUIREMENT="+REQ)

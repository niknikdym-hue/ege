...
ACCEPTANCE = HERE / "RU11-DESCRIPTION-IDENTIFICATION-BOUNDED-SUBJECT-SEMANTIC-ACCEPTANCE-v0.1.json"
...
CONTENT = PROGRAM / "production-learning-content/RU-PROG-11-DESCRIPTION-IDENTIFICATION-WAVE-007-v0.1.json"

CANDIDATE = "candidate-045"
TAXONOMY = "description_identification"
SEMANTIC = "ru-text-description-identification"
CANONICAL_LABEL = "Распознавание описания"
EXPECTED_CHECK_IDS = {"p11-desc-v1", "p11-desc-v2"}
EXPECTED_BROADER_CHECK_IDS = {"p11-u3-v2"}
...
def main() -> int:
    ...
    if acceptance.get("status") != "CENTRAL_BRAIN_ACCEPTED_RU11_DESCRIPTION_IDENTIFICATION_BOUNDED_SUBJECT_SEMANTIC" or acceptance.get("authority_issue") != 161:
        raise AssertionError("RU11 candidate-045 acceptance authority/status drift")
    if acceptance.get("canonical_school_registry_mutated") is not False or acceptance.get("new_parallel_registry_created") is not False:
        raise AssertionError("RU11 candidate-045 registry boundary drift")

    sg = acceptance.get("source_truth_guard") or {}
    if sg.get("inventory_candidate_review_status") != "draft" or sg.get("inventory_candidate_status_silently_upgraded") is not False or sg.get("taxonomy_backing_review_status") != "source_verified" or sg.get("skill_graph_evidence_status") != "confirmed":
        raise AssertionError("RU11 candidate-045 source guard drift")
    if sg.get("accepted_positive_scope") != [
        "system of simultaneous features of one object, place, person or state",
        "spatial, qualitative or static organization without event development as the semantic center",
    ]:
        raise AssertionError("RU11 candidate-045 positive scope drift")
    if sg.get("narration_mastery_admitted") is not False or sg.get("reasoning_mastery_admitted") is not False or sg.get("generic_combined_speech_type_mastery_admitted") is not False:
        raise AssertionError("RU11 candidate-045 adjacent mastery guard drift")
    ...
    for key in ("exact_current_source_identity_required","source_verified_taxonomy_backing_required","confirmed_skill_graph_evidence_required","current_missing_subject_candidate_required","draft_candidate_status_must_be_preserved","content_adequacy_review_required","exact_school_meaning_collision_forbidden","reuse_first","component_specific_independent_evidence_required","school_duplicate_forbidden"):
        if policy.get(key) is not True:
            raise AssertionError(f"RU11 candidate-045 policy weakened: {key}")
    if policy.get("minimum_component_specific_independent_checks") != 2 or policy.get("new_subject_identity_namespace") != "ru-*":
        raise AssertionError("RU11 candidate-045 component/namespace policy drift")
    for key in ("ege_taxonomy_id_promoted_unchanged","candidate_id_used_as_semantic_id","adjacent_candidates_admitted","narration_mastery_admitted_by_this_authority","reasoning_mastery_admitted_by_this_authority","semantic_relation_mastery_admitted_by_this_authority","generic_task24_result_can_emit_exact_component_mastery","combined_speech_type_result_can_emit_exact_component_mastery","adjective_count_or_marker_word_can_emit_exact_component_mastery","subject_semantic_acceptance_can_reduce_object_counts_without_exact_binding","content_presence_alone_is_semantic_admission"):
        if policy.get(key) is not False:
            raise AssertionError(f"RU11 candidate-045 fail-closed policy drift: {key}")
    ...
    candidate = one([r for r in objects if r.get("source_system") == "semantic_candidate" and r.get("source_id") == CANDIDATE and r.get("authority_status") == "current"], "RU11 candidate-045")
    if candidate.get("audit_classification") != "MISSING_SUBJECT_SEMANTIC_CANDIDATE" or candidate.get("candidate_canonical_owner") != CANDIDATE or candidate.get("current_semantic_refs") != [TAXONOMY] or candidate.get("review_status") != "draft":
        raise AssertionError("RU11 candidate-045 inventory ownership/status drift")

    backing = one([r for r in objects if r.get("source_system") == "ege_skill_graph" and r.get("source_id") == TAXONOMY and r.get("authority_status") == "current" and r.get("candidate_canonical_owner") == CANDIDATE], "RU11 candidate-045 backing")
    if backing.get("review_status") != "source_verified" or backing.get("audit_classification") != "EGE_TAXONOMY_NODE" or normalized(backing.get("observed_meaning")) != normalized(candidate.get("observed_meaning")):
        raise AssertionError("RU11 candidate-045 taxonomy backing drift")
    skill = one([r for r in graph.get("skills", []) if isinstance(r, dict) and r.get("skill_id") == TAXONOMY], "RU11 description graph node")
    if skill.get("evidence_status") != "confirmed" or skill.get("parent_skill_id") != "speech_type_analysis" or skill.get("exam_task_numbers") != [24] or skill.get("name_ru") != CANONICAL_LABEL or normalized(skill.get("description")) != normalized(candidate.get("observed_meaning")):
        raise AssertionError("RU11 description graph/source meaning drift")

    if any(SEMANTIC in {str(ref) for ref in (r.get("current_semantic_refs") or [])} for r in objects if r.get("authority_status") == "current"):
        raise AssertionError("RU11 candidate-045 semantic collision")
    if any(r.get("source_system") == "school_canonical" and r.get("authority_status") == "current" and normalized(r.get("observed_meaning")) == normalized(candidate.get("observed_meaning")) for r in objects):
        raise AssertionError("RU11 candidate-045 exact school meaning exists; reuse required")

    if review.get("status") != "CENTRAL_BRAIN_RU11_DESCRIPTION_IDENTIFICATION_CONTENT_ADEQUACY_REVIEW_COMPLETE_NO_ADMISSION":
        raise AssertionError("RU11 description review status drift")
    rd = review.get("review_decision") or {}
    if rd.get("content_exact_for_current_candidate_045_source_meaning") is not True or rd.get("source_candidate_inventory_status_preserved_as_draft") is not True or rd.get("semantic_admission_by_this_review") is not False or rd.get("object_level_admission_units_closed") != 0 or rd.get("object_level_requirements_closed") != 0 or rd.get("false_exact_mastery_admissions") != 0 or rd.get("next_status") != "READY_FOR_SEPARATE_BOUNDED_SUBJECT_SEMANTIC_ACCEPTANCE_WITH_DRAFT_CANDIDATE_GUARD":
        raise AssertionError("RU11 description content review decision drift")
    ...
    direct_broader_ids = {str(r.get("id")) for r in broad_checks if str(r.get("id")) in EXPECTED_BROADER_CHECK_IDS}
    if direct_broader_ids != EXPECTED_BROADER_CHECK_IDS:
        raise AssertionError(f"RU11 candidate-045 reuse-gap drift: {sorted(direct_broader_ids)}")
    reuse = acceptance.get("reuse_first_decision") or {}
    if reuse.get("broader_existing_unit_present") is not True or reuse.get("broader_existing_unit_reused_as_scope_and_explanation_evidence") is not True or reuse.get("broader_existing_unit_is_exact_candidate_045_mastery_evidence") is not False or reuse.get("broader_existing_unit_direct_description_independent_checks") != 1 or reuse.get("minimum_component_specific_independent_checks_for_this_acceptance") != 2 or reuse.get("new_content_materialized_only_for_proven_component_verification_gap") is not True:
        raise AssertionError("RU11 candidate-045 reuse decision drift")

    if content.get("status") != "SUBJECT_ACCEPTANCE_REQUIRED" or content.get("module_id") != "RU-PROG-11":
        raise AssertionError("RU11 description content status/module drift")
    ...
    if guard.get("source_passages_copied") != 0 or guard.get("commercial_textbook_bytes") != 0 or guard.get("learner_examples") != "ORIGINAL_EKSAMIO":
        raise AssertionError("RU11 description copyright guard drift")
    content_reuse = content.get("reuse_review") or {}
    if content_reuse.get("existing_unit_reused_as_exact_candidate_045_mastery_evidence") is not False or content_reuse.get("broader_existing_unit_direct_description_independent_checks") != 1 or content_reuse.get("new_content_materialized_only_for_proven_component_verification_gap") is not True:
        raise AssertionError("RU11 description content reuse guard drift")

    unit = one([r for r in content.get("units", []) if isinstance(r, dict) and r.get("proposed_semantic_id") == SEMANTIC], "RU11 description learner unit")
    if unit.get("title_ru") != CANONICAL_LABEL:
        raise AssertionError("RU11 description learner title drift")
    for key, minimum in (("decision_algorithm",6),("worked_examples",4),("misconceptions",4),("guided_practice",3),("independent_practice",3),("mixed_transfer_practice",2),("retention_items",2),("independent_verification",2)):
        if not isinstance(unit.get(key), list) or len(unit[key]) < minimum:
            raise AssertionError(f"RU11 description learner section incomplete: {key}")
    checks = [r for r in (unit.get("independent_verification") or []) if isinstance(r, dict)]
    if {r.get("id") for r in checks} != EXPECTED_CHECK_IDS or len(checks) != 2 or any(r.get("type") != "constructed_response" for r in checks) or any((r.get("scoring") or {}).get("max_points") != 3 for r in checks) or any(len((r.get("scoring") or {}).get("criteria") or []) != 3 for r in checks):
        raise AssertionError("RU11 description exact verification drift")
    peis = unit.get("peis_evidence") or {}
    if peis.get("semantic_ref_status") != "PROPOSED_NOT_CANONICAL" or peis.get("source_candidate_review_status") != "draft" or peis.get("independent_verification_required") is not True or peis.get("exact_mastery_requires_two_component_specific_description_checks") is not True or peis.get("generic_task24_result_can_emit_exact_component_mastery") is not False or peis.get("broader_combined_unit_can_emit_exact_description_mastery") is not False:
        raise AssertionError("RU11 description PEIS boundary drift")
    forbidden = " ".join((unit.get("tutor_grounding") or {}).get("forbidden") or []).lower()
    if "generic task-24" not in forbidden or "narration" not in forbidden or "reasoning" not in forbidden:
        raise AssertionError("RU11 description Tutor guard drift")

    decisions = acceptance.get("decisions")
    if not isinstance(decisions, list) or len(decisions) != 1:
        raise AssertionError("RU11 description acceptance must contain one decision")
    d = decisions[0]
    if d.get("candidate_ref") != CANDIDATE or d.get("source_taxonomy_id") != TAXONOMY or d.get("accepted_semantic_id") != SEMANTIC or d.get("canonical_label_ru") != CANONICAL_LABEL or d.get("subject_semantic_status") != "CENTRAL_BRAIN_ACCEPTED_BOUNDED_SUBJECT_SEMANTIC" or d.get("source_evidence_status") != "confirmed" or d.get("source_candidate_inventory_review_status") != "draft" or d.get("excluded_adjacent_candidate_refs") != ADJACENT or d.get("object_binding_status") != "NOT_BOUND_TO_ANY_EXACT_ADMISSION_UNIT_OR_REQUIREMENT":
        raise AssertionError("RU11 description acceptance crosswalk drift")
    ...
    if summary.get("accepted_bounded_subject_semantics") != 1 or summary.get("accepted_ru_subject_semantics") != 1 or summary.get("adjacent_candidates_admitted") != 0 or summary.get("new_school_canonical_identities") != 0 or summary.get("object_level_admission_units_closed") != 0 or summary.get("object_level_requirements_closed") != 0 or summary.get("false_exact_mastery_admissions") != 0:
        raise AssertionError("RU11 description acceptance summary drift")

    digest = hashlib.sha256(json.dumps(acceptance, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
    print("RU11_DESCRIPTION_IDENTIFICATION_BOUNDED_SUBJECT_SEMANTIC_ACCEPTANCE=PASS")
    print(f"SOURCE_CANDIDATE_REVIEW_STATUS={candidate.get('review_status')}")
    print(f"ACCEPTED_SEMANTIC={SEMANTIC}")
    print(f"BROADER_EXISTING_DIRECT_DESCRIPTION_CHECKS={len(direct_broader_ids)}")

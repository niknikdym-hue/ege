#!/usr/bin/env python3
"""Current Sep-1 Russian launch progress with seventh exact RU01 phonetics object acceptance."""
...
BASE=H/"build_russian_semantic_acceptance_progress_launch_current_v9.py"
...
AUTH=H/"RU01-PHONETICS-OGE-P021-4-1-7-EXACT-CANONICAL-COMPONENT-ACCEPTANCE-v0.1.json"
CONTENT=H.parent/"production-learning-content"/"RU-PROG-01-PHONETICS-GRAPHICS-WAVE-001-v0.1.json"
BASE_HEAD="c1724f9ef1435baccd9f2a0435ce486a4ffa0485"
BASE_BLOB="c52bb88546853e1f0176d769e0599259c3e88dfd"
BASE_SHA="7a43d31c1bef4e66d57904920b69c943f8b46a7c1f1f9f9d98556ef0fa1aeb34"
...
AUTH_SHA="ee2ee9b43050513718f9edb73a9323dc03983a9d86add498a2446e98c7475c3c"
AUTH_ID="RU01_PHONETICS_OGE_P021_4_1_7_EXACT_CANONICAL_COMPONENT_ACCEPTANCE_v0.1"
UNIT="RAU-f3896151ffac143c05da"
REQ="RSK-OGE_COD-4-1-7-P021"
GROUP="RUS-SEM-REVIEW-001"
SOURCE="FIPI-OGE-RU-2026-FINAL"
DOC="OGE_COD"
PAGE=21
CODE="4.1.7"
LOCATOR="FIPI-OGE-RU-2026-FINAL/OGE_COD p.21 4.1.7"
...
"RU01_PHONETICS_OGE_P014_3_1_EXACT_CANONICAL_COMPONENT_ACCEPTANCE_v0.1":("RAU-df45aa3b99e1baefca01","RSK-OGE_COD-3-1-P014")
}
EXPECTED_BASE={
...
def build_progress()->dict[str,Any]:
    if blob(BASE)!=BASE_BLOB:
        raise ValueError("current-v9 builder blob drift")
    d=runpy.run_path(str(BASE))["build_progress"]()
    if d.get("schema_version")!="0.12.0" or d.get("normalized_sha256")!=BASE_SHA:
        raise ValueError("current-v9 authority drift")
    s=d["progress_summary"]
    for k,v in EXPECTED_BASE.items():
        if s.get(k)!=v:
            raise ValueError(f"current-v9 aggregate drift: {k}")
    if len(d.get("accepted_authorities",[]))!=56:
        raise ValueError("current-v9 authority count drift")
    ...
    if UNIT not in ready.get("admission_unit_ids",[]) or REQ not in ready.get("requirement_ids",[]):
        raise ValueError("OGE P021 4.1.7 no longer exact-reuse-ready")
    rec=[x for x in b.get("records",[]) if x.get("admission_unit_id")==UNIT and x.get("requirement_id")==REQ]
    if len(rec)!=1:
        raise ValueError("OGE P021 4.1.7 binding not unique")
    r=rec[0]
    for k,v in {"source_id":SOURCE,"document_id":DOC,"page":PAGE,"code":CODE,"source_locator":LOCATOR,
        ...
        if r.get(k)!=v:
            raise ValueError(f"OGE P021 4.1.7 binding identity drift: {k}")
    ...
    if a.get("status")!="CENTRAL_BRAIN_ACCEPTED_EXACT_RU01_PHONETICS_WORD_ANALYSIS_CANONICAL_COMPONENT_SET":
        raise ValueError("OGE P021 4.1.7 authority status drift")
    if (a.get("current_launch_progress_v9_base_head_sha"),a.get("current_launch_progress_v9_builder_git_blob_sha1"),a.get("current_launch_progress_v9_normalized_sha256"))!=(BASE_HEAD,BASE_BLOB,BASE_SHA):
        raise ValueError("OGE P021 4.1.7 base binding drift")
    if (a.get("exact_object_binding_review_sha256"),a.get("exact_source_review_input_sha256"))!=(BINDING_SHA,BINDING_INPUT_SHA):
        raise ValueError("OGE P021 4.1.7 source binding drift")
    ...
    for k,v in exact.items():
        if dec.get(k)!=v:
            raise ValueError(f"OGE P021 4.1.7 authority identity drift: {k}")
    ...
    if m.get("component_specific_independent_evidence_required") is not True or m.get("validated_word_analysis_item_may_support_only_its_single_canonical_ref") is not True:
        raise ValueError("OGE P021 4.1.7 evidence boundary weakened")
    for k in ("generic_ru01_attempt_can_emit_exact_component_mastery","spelling_or_morphemic_evidence_can_substitute_for_word_analysis","shared_evidence_can_close_sibling_source_objects_without_separate_acceptance"):
        if m.get(k) is not False:
            raise ValueError(f"OGE P021 4.1.7 mastery boundary weakened: {k}")
    ...
    if UNIT not in set(map(str,g.get("admission_unit_ids",[]))):
        raise ValueError("OGE P021 4.1.7 unit missing from group")
    rows=[x for x in g.get("requirements",[]) if x.get("requirement_id")==REQ]
    if len(rows)!=1:
        raise ValueError("OGE P021 4.1.7 requirement not unique")
    for k,v in (("source_id",SOURCE),("document_id",DOC),("page",PAGE),("code",CODE),("source_locator",LOCATOR)):
        if rows[0].get(k)!=v:
            raise ValueError(f"OGE P021 4.1.7 source identity drift: {k}")
    ...
    if any(x.get("admission_unit_id")==UNIT or x.get("requirement_id")==REQ for x in accepted):
        raise ValueError("OGE P021 4.1.7 already accepted")
    if any(x.get("id")==AUTH_ID for x in d.get("accepted_authorities",[])):
        raise ValueError("OGE P021 4.1.7 authority already integrated")
    ...
    if g["accepted_component_set_count"]!=7:
        raise ValueError("OGE P021 4.1.7 accepted-set count drift")
    ...
    for k,v in EXPECTED_AFTER.items():
        if s.get(k)!=v:
            raise ValueError(f"OGE P021 4.1.7 post-acceptance aggregate drift: {k}")
    if len(d["accepted_authorities"])!=57:
        raise ValueError("OGE P021 4.1.7 post-acceptance authority count drift")

    siblings=set(ready.get("admission_unit_ids",[]))-{UNIT}-{u for u,_ in PRIOR.values()}
    if siblings:
        raise ValueError(f"unexpected remaining READY sibling set: {sorted(siblings)}")
    new=[x for x in g["accepted_component_sets"] if x.get("accepted_authority_id")==AUTH_ID]
    if len(new)!=1 or new[0].get("admission_unit_id")!=UNIT:
        raise ValueError("OGE P021 4.1.7 acceptance projection drift")
    if s.get("false_exact_mastery_admissions")!=0:
        raise ValueError("false exact mastery must remain zero")

    d["schema_version"]="0.13.0"
    d["base_current_launch_progress_v9_head_sha"]=BASE_HEAD
    d["base_current_launch_progress_v9_builder_git_blob_sha1"]=BASE_BLOB
    d["base_current_launch_progress_v9_normalized_sha256"]=BASE_SHA
...
def main()->int:
    p=argparse.ArgumentParser();p.add_argument("--output");p.add_argument("--emit",action="store_true");a=p.parse_args()
    d=build_progress()
    if a.output:
    ...
    if a.emit:
        ...
        s=d["progress_summary"]
        print("RUSSIAN_RU01_OGE_P021_4_1_7_EXACT_OBJECT_ACCEPTANCE=PASS")
        print("ACCEPTED_UNIT="+UNIT);print("ACCEPTED_REQUIREMENT="+REQ)
        print("EXACT_COMPONENT_REFS=1");print("INDEPENDENT_EVIDENCE_ITEMS=2")
        print("ADDITIONAL_SIBLING_RU01_OBJECTS_ACCEPTED=0");print("REMAINING_EXACT_REUSE_READY_RU01_OBJECTS=0")

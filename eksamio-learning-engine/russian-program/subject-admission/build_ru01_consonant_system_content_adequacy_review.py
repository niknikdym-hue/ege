...
PROGRAM = HERE.parent
CONTENT = PROGRAM / "production-learning-content/RU-PROG-01-CONSONANT-SYSTEM-WAVE-008-v0.1.json"

SEMANTIC = "ru-phonetics-consonant-system"
TAXONOMY = "consonant_system"
SOURCE_CLAUSES = ["EDSOO59-P187-4.1.2", "OGE-COD-P020-4.1.2"]
VERIFICATION_IDS = ["p01-u10-v1", "p01-u10-v2", "p01-u10-v3", "p01-u10-v4", "p01-u10-v5"]
CONTENT_GIT_BLOB = "3fd8716766da403ccd3bcfd3cca2d97af5631db6"
READINESS_GATE = {
    "workflow": "Russian RU01 consonant system candidate content readiness",
    "run_id": 34416742407,
    "head_sha": "a3d5f4c0c78298b95ea59c79209d76708d2a2925",
    "conclusion": "SUCCESS",
}
...
def build_review() -> dict[str, Any]:
    ...
    require(git_blob_sha(raw) == CONTENT_GIT_BLOB, "consonant-system content blob drift")
    ...
    require("narrower" in coverage_note or "partial" in coverage_note, "partial-current-owner boundary missing")
    ...
    require(by_doc["EDSOO59"].get("source_locator") == "EDSOO-RU-5-9-2025/EDSOO59 p.187 4.1.2", "EDSOO locator drift")
    require(by_doc["OGE_COD"].get("source_locator") == "FIPI-OGE-RU-2026-FINAL/OGE_COD p.20 4.1.2", "OGE locator drift")
    require({row.get("official_requirement") for row in official} == {"Система согласных звуков"}, "official requirement drift")
    ...
    require(len(units) == 1, "exactly one bounded consonant-system unit required")
    unit = units[0]
    ...
    for token in ("согласн", "букв", "звонк", "глух", "твёрд", "мягк", "пар", "непар", "удар", "произнош"):
    ...
    require("отдель" in explanation_text or "локаль" in explanation_text or "одн" in explanation_text, "single-feature versus full-system separation missing")
    ...
    require("согласн" in algorithm_text and "букв" in algorithm_text and "систем" in algorithm_text, "decision algorithm scope incomplete")
    require("звонк" in algorithm_text and "глух" in algorithm_text, "voicing algorithm boundary incomplete")
    require("твёрд" in algorithm_text and "мягк" in algorithm_text, "hardness algorithm boundary incomplete")
    require("пар" in algorithm_text and "непар" in algorithm_text, "pairing algorithm boundary incomplete")
    ...
    v1 = next(row for row in verification if row.get("id") == "p01-u10-v1")
    require(set(map(str, v1.get("correct") or [])) == {"[м]", "[т]", "[й’]"}, "consonant-member evidence drift")
    v2 = next(row for row in verification if row.get("id") == "p01-u10-v2")
    v2_text = (str(v2.get("prompt") or "") + " " + " ".join(map(str, v2.get("options") or []))).lower()
    require("букв" in v2_text and "звук" in v2_text, "sound-letter evidence boundary drift")
    v3 = next(row for row in verification if row.get("id") == "p01-u10-v3")
    v3_text = (str(v3.get("prompt") or "") + " " + " ".join(map(str, v3.get("options") or []))).lower()
    require("звонк" in v3_text and "глух" in v3_text and "[д] — [т]" in v3_text, "voicing-pair evidence drift")
    v4 = next(row for row in verification if row.get("id") == "p01-u10-v4")
    v4_text = (str(v4.get("prompt") or "") + " " + " ".join(map(str, v4.get("options") or []))).lower()
    require("твёрд" in v4_text and "мягк" in v4_text and "[м] — [м’]" in v4_text, "hardness-pair evidence drift")
    v5 = next(row for row in verification if row.get("id") == "p01-u10-v5")
    v5_text = (str(v5.get("prompt") or "") + " " + " ".join(map(str, v5.get("options") or []))).lower()
    require("систем" in v5_text and "отдель" in v5_text and "произнош" in v5_text, "full-system boundary evidence drift")
    ...
    result: dict[str, Any] = {
        ...
        "status": "CENTRAL_BRAIN_RU01_CONSONANT_SYSTEM_CONTENT_ADEQUACY_REVIEW_COMPLETE_NO_ADMISSION",
        "authority_pr": 164,
        ...
        "learner_content": {
            "path": "russian-program/production-learning-content/RU-PROG-01-CONSONANT-SYSTEM-WAVE-008-v0.1.json",
        ...
        "adequacy_decision": {
            "content_exact_for_bounded_consonant_system_candidate": True,
            "official_clause_scope_covered": True,
            "consonant_sounds_and_letters_separated": True,
            "voicing_and_devoicing_system_covered": True,
            "hardness_and_softness_system_covered": True,
            "paired_and_unpaired_cases_covered": True,
            "partial_vowel_consonant_feature_owner_not_promoted_to_full_system": True,
            "vowel_system_separated": True,
...
def main() -> int:
    ...
    if args.emit:
        ...
        print("RU01_CONSONANT_SYSTEM_CONTENT_ADEQUACY=PASS")

#!/usr/bin/env python3
"""Current Sep-1 Russian launch progress after bounded RU01 vowel-system semantic acceptance."""
...
BASE = HERE / "build_russian_semantic_acceptance_progress_launch_current_v21.py"
...
AUTHORITY_ID = "RU01_VOWEL_SYSTEM_BOUNDED_SUBJECT_SEMANTIC_ACCEPTANCE_v0.1"
...
EXPECTED_BASE = {
    ...
    "accepted_bounded_ru_subject_semantics": 71,
    "accepted_bounded_ru_semantics_total": 80,
    "false_exact_mastery_admissions": 0,
}
EXPECTED_AFTER = dict(EXPECTED_BASE)
EXPECTED_AFTER["accepted_bounded_ru_subject_semantics"] = 72
EXPECTED_AFTER["accepted_bounded_ru_semantics_total"] = 81
...
def build_progress() -> dict[str, Any]:
    ...
    if data.get("schema_version") != "0.23.0":
        raise ValueError("current-v21 schema drift")
    summary = data.get("progress_summary") or {}
    for key, expected in EXPECTED_BASE.items():
        if summary.get(key) != expected:
            raise ValueError(f"current-v21 aggregate drift: {key}")
    accepted_authorities = data.get("accepted_authorities") or []
    if len(accepted_authorities) != 67:
        raise ValueError("current-v21 accepted authority count drift")
    predecessor = data.get("newly_accepted_bounded_subject_semantic") or {}
    if predecessor.get("semantic_id") != "ru-phonetics-stress":
        raise ValueError("current-v21 predecessor semantic drift")
    if predecessor.get("independent_verification_item_ids") != [f"p01-u8-v{i}" for i in range(1, 6)]:
        raise ValueError("current-v21 predecessor evidence drift")
    if predecessor.get("parent_object_count_requiring_future_complete_component_set_acceptance") != 2:
        raise ValueError("current-v21 parent-object frontier drift")
    ...
    if adequacy.get("status") != "CENTRAL_BRAIN_RU01_VOWEL_SYSTEM_CONTENT_ADEQUACY_REVIEW_COMPLETE_NO_ADMISSION":
        raise ValueError("vowel-system adequacy predecessor drift")
    candidate = adequacy.get("candidate") or {}
    if candidate.get("semantic_id") != SEMANTIC or candidate.get("source_taxonomy_id") != "vowel_system":
        raise ValueError("vowel-system candidate identity drift")
    if candidate.get("source_clause_ids") != ["EDSOO59-P187-4.1.1", "OGE-COD-P020-4.1.1"]:
        raise ValueError("vowel-system candidate source-clause drift")
    learner = adequacy.get("learner_content") or {}
    if learner.get("git_blob_sha1") != "37bbbb2793915ab45142cbb758b201c5ac8129be":
        raise ValueError("vowel-system learner content blob drift")
    if learner.get("component_specific_verification_ids") != VERIFICATION_IDS:
        raise ValueError("vowel-system evidence identity drift")
    for key in (
        ...
        if learner.get(key) is not True:
            raise ValueError(f"vowel-system PEIS boundary drift: {key}")
    ...
    if adequacy_decision.get("next_status") != "READY_FOR_SEPARATE_BOUNDED_SUBJECT_SEMANTIC_ACCEPTANCE":
        raise ValueError("vowel-system adequacy next-state drift")
    if adequacy_decision.get("semantic_admission_by_this_review") is not False:
        raise ValueError("vowel-system adequacy self-admitted semantic")
    if adequacy_decision.get("object_level_admission_units_closed") != 0 or adequacy_decision.get("object_level_requirements_closed") != 0:
        raise ValueError("vowel-system adequacy self-closed parent object")
    if adequacy_decision.get("false_exact_mastery_admissions") != 0:
        raise ValueError("vowel-system adequacy false exact mastery drift")
    for key in (
        ...
        if adequacy_decision.get(key) is not True:
            raise ValueError(f"vowel-system adequacy boundary weakened: {key}")

    acceptance = json.loads(ACCEPTANCE.read_text(encoding="utf-8"))
    if acceptance.get("status") != "CENTRAL_BRAIN_ACCEPTED_RU01_VOWEL_SYSTEM_BOUNDED_SUBJECT_SEMANTIC":
        raise ValueError("vowel-system semantic acceptance status drift")
    ...
    if policy.get("parent_object_can_close_before_all_required_clauses_have_exact_accepted_owners_and_component_specific_evidence") is not False:
        raise ValueError("vowel-system parent-object early closure opened")
    if policy.get("generic_ru01_result_can_emit_exact_component_mastery") is not False:
        raise ValueError("generic RU01 exact vowel-system mastery opened")
    for key in (
        ...
        if policy.get(key) is not False:
            raise ValueError(f"vowel-system adjacent semantic substitution opened: {key}")
    if (acceptance.get("summary") or {}).get("false_exact_mastery_admissions") != 0:
        raise ValueError("vowel-system semantic false exact mastery drift")

    if any(row.get("id") == AUTHORITY_ID for row in accepted_authorities if isinstance(row, dict)):
        raise ValueError("vowel-system semantic authority already integrated")
    ...
    for target in TARGETS:
        if any(
            ...
            raise ValueError("vowel-system parent object exact-bound before all broad-header components are accepted")
    ...
    for target in TARGETS:
        ...
        if len(matches) != 1:
            raise ValueError(f"vowel-system parent requirement missing or duplicated: {target['requirement_id']}")
        if matches[0].get("source_locator") != target["source_locator"]:
            raise ValueError(f"vowel-system parent source locator drift: {target['requirement_id']}")
    ...
    for key, expected in EXPECTED_AFTER.items():
        if summary.get(key) != expected:
            raise ValueError(f"post-vowel-system aggregate drift: {key}")
    if len(accepted_authorities) != 68:
        raise ValueError("post-vowel-system authority count drift")

    base_sha = data.get("normalized_sha256")
    if not isinstance(base_sha, str) or len(base_sha) != 64:
        raise ValueError("current-v21 normalized SHA missing")
    data["schema_version"] = "0.24.0"
    data["base_current_launch_progress_v21_normalized_sha256"] = base_sha
    data["newly_accepted_bounded_subject_semantic"] = {
        "authority_id": AUTHORITY_ID,
        "semantic_id": SEMANTIC,
        "authority_sha256": authority_sha,
        "independent_verification_item_ids": VERIFICATION_IDS,
        "source_clause_ids": [target["vowel_system_clause_id"] for target in TARGETS],
        ...
        "parent_objects_remain_pending": [
            {
                ...
                "vowel_system_clause_id": target["vowel_system_clause_id"],
            }
            for target in TARGETS
        ],
        "adjacent_semantics_remain_separate": [
            "ru-phonetics-vowel-consonant-features",
            "consonant-system-unresolved",
    ...
    data.setdefault("policy", {})["ru01_vowel_system_semantic_acceptance_requires_component_specific_evidence"] = True
    data["policy"]["ru01_vowel_system_semantic_acceptance_can_close_parent_object"] = False
    data["policy"]["generic_ru01_attempt_can_emit_exact_vowel_system_mastery"] = False
    data["policy"]["partial_vowel_consonant_feature_can_substitute_for_full_vowel_system"] = False
    data["policy"]["consonant_sound_letter_composition_stress_or_pronunciation_can_substitute_for_vowel_system"] = False
...
def main() -> int:
    ...
    if args.emit:
        ...
        print("RUSSIAN_RU01_VOWEL_SYSTEM_BOUNDED_SUBJECT_SEMANTIC_ACCEPTANCE=PASS")

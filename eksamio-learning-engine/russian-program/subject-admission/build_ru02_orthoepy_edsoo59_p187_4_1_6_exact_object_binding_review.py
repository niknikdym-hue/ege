#!/usr/bin/env python3
"""Fail-closed exact object-binding review for EDSOO59 p.187 4.1.6 RU02 orthoepy."""
...
CURRENT = HERE / "build_russian_semantic_acceptance_progress_launch_current_v26.py"
...
PRIOR_OBJECT_ACCEPTANCE = HERE / "RU02-ORTHOEPY-EXACT-CANONICAL-COMPONENT-ACCEPTANCE-v0.1.json"

CURRENT_HEAD = "4f43c838e83639827a040f1d8a8d75048e19abf2"
CURRENT_RUN_ID = 34543590950
CURRENT_SHA = "2b2ac9e07a98ac6b988e26bacb5b6e1a22394233c4f83a123354f3dcbd9ec402"

GROUP = "RUS-SEM-REVIEW-047"
TARGET_UNIT = "RAU-da83bb720dea792b3afb"
TARGET_REQUIREMENT = "RSK-EDSOO59-4-1-6-P187"
TARGET_SOURCE = "EDSOO-RU-5-9-2025"
TARGET_DOCUMENT = "EDSOO59"
TARGET_PAGE = 187
TARGET_CODE = "4.1.6"
TARGET_LOCATOR = "EDSOO-RU-5-9-2025/EDSOO59 p.187 4.1.6"
...
PRIOR_REQUIREMENT = "RSK-EDSOO1011-2-2-4-P074"
...
def build_review() -> dict[str, Any]:
    ...
    if current.get("schema_version") != "0.28.0":
    ...
    if len(requirements) != 1:
        raise ValueError("target requirement missing or duplicated in current-v26")
    req = requirements[0]
    expected_req = {
    ...
    if len(accepted_sets) != 1:
        raise ValueError("expected exactly one prior RU02 accepted object")
    prior = accepted_sets[0]
    if prior.get("admission_unit_id") != PRIOR_UNIT or prior.get("requirement_id") != PRIOR_REQUIREMENT:
        raise ValueError("prior RU02 exact object drift")
    ...
    prior_authority = load(PRIOR_OBJECT_ACCEPTANCE)
    if prior_authority.get("status") != "CENTRAL_BRAIN_ACCEPTED_EXACT_RU02_ORTHOEPY_CANONICAL_COMPONENT_SET":
        raise ValueError("prior RU02 object acceptance status drift")
    prior_decision = prior_authority.get("decision") or {}
    if prior_decision.get("canonical_component_refs") != COMPONENTS:
        raise ValueError("prior accepted RU02 component refs drift")
    if prior_decision.get("component_specific_independent_evidence") != EVIDENCE:
        raise ValueError("prior accepted RU02 component evidence drift")
    if prior_decision.get("independent_evidence_items") != 9:
        raise ValueError("prior accepted RU02 evidence count drift")
    if (prior_authority.get("summary") or {}).get("false_exact_mastery_admissions") != 0:
        raise ValueError("prior RU02 object acceptance opened false exact mastery")
    ...
    result: dict[str, Any] = {
        ...
        "status": "CENTRAL_BRAIN_RU02_EDSOO59_P187_4_1_6_EXACT_OBJECT_BINDING_REVIEW_READY_FOR_SEPARATE_ACCEPTANCE_NOT_ACCEPTED",
        "current_v26_exact_head_gate": {
        ...
        "reuse_first_owner_resolution": {
            ...
            "source_backing": (
                "The exact current source row is grouped under the normalized official meaning "
                "'Применять нормативное произношение и ударение.'; the two current accepted bounded "
                "RU02 semantic owners independently cover pronunciation selection and stress selection."
            ),
        ...
        "sibling_reuse_boundary": {
            "prior_accepted_sibling_admission_unit_id": PRIOR_UNIT,
            "prior_accepted_sibling_requirement_id": PRIOR_REQUIREMENT,
        ...
        "acceptance_readiness": {
            ...
            "target_still_in_current_v26_remainder": True,
        ...
        "summary": {
            ...
            "false_exact_mastery_admissions": 0,
            "current_subject_review_units_remaining": 1273,
            "current_subject_review_requirements_remaining": 1348,
        },
    }
...
def main() -> int:
    ...
    if args.emit:
        ...
        print("RU02_EDSOO59_P187_4_1_6_EXACT_OBJECT_BINDING_REVIEW=PASS")

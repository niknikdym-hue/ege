...
COMPONENT_VALIDATOR = HERE / "validate_oge_6_6_component_evidence.py"
...
def build_audit() -> dict[str, Any]:
    ...
    if route.get("status") != "CURRENT_OGE_2026_6_6_ROUTE_SUPERSESSION_EXACT_OWNER_FRONTIER_NO_OBJECT_ADMISSION":
        raise ValueError("OGE 6.6 current route is not the fail-closed exact-owner supersession")
    owners = [str(ref) for ref in route.get("exact_owner_refs") or []]
    if len(owners) != 10 or len(set(owners)) != 10:
        raise ValueError("OGE 6.6 exact owner frontier must contain 10 unique refs")
    if route.get("mastery_boundary", {}).get("route_attempt_can_emit_exact_component_mastery") is not False:
        raise ValueError("OGE 6.6 route mastery boundary weakened")
    if route.get("mastery_boundary", {}).get("component_specific_independent_evidence_required") is not True:
        raise ValueError("OGE 6.6 component-specific evidence requirement weakened")

    validation_summary = component_validation.get("summary") or {}
    if validation_summary.get("exact_owner_frontier") != 10:
        raise ValueError("component evidence validator owner frontier drift")
    if validation_summary.get("owners_with_valid_component_evidence") != 10:
    ...
    if len(packet_matches) != 1:
        raise ValueError(f"expected one exact OGE_COD 6.6 requirement, got {len(packet_matches)}")
    ...
    if len(accounting_matches) != 1:
        raise ValueError("OGE 6.6 requirement must map to exactly one accounting unit")
    accounting_row = accounting_matches[0]
    if len(accounting_row.get("members") or []) != 1:
        raise ValueError("OGE 6.6 accounting unit must be single-member before component acceptance")
    if accounting_row.get("disposition") != "PARTIAL_OR_COMPOSITE":
        raise ValueError("OGE 6.6 pre-acceptance disposition drift")
    if accounting_row.get("semantic_identity_ref") is not None:
        raise ValueError("OGE 6.6 must not already carry a singular semantic identity")
    ...
    result: dict[str, Any] = {
        ...
        "date": "2026-08-31",
        "status": (
            "CENTRAL_BRAIN_OGE_6_6_COMPONENT_EVIDENCE_FRONTIER_COMPLETE_READY_FOR_SEPARATE_OBJECT_ACCEPTANCE"
            if ready
            else "CENTRAL_BRAIN_OGE_6_6_COMPONENT_EVIDENCE_GAPS_PROVEN_NO_OBJECT_ACCEPTANCE"
        ),
        "scope": "OGE_2026_CONTENT_CODE_6_6_EXPLICIT_COMPONENT_EVIDENCE_AUDIT",
        "policy": {
            "reuse_first": True,
            ...
            "exact_component_evidence_rule": (
                "A current validated independent item must explicitly reference exactly one OGE 6.6 canonical owner. "
        ...
        "component_evidence": {
            "path": "russian-program/production-learning-content/RU-PROG-08-OGE-6.6-COMPONENT-EVIDENCE-WAVE-001-v0.1.json",
            "validator_path": "russian-program/subject-admission/validate_oge_6_6_component_evidence.py",
...
def main() -> int:
    ...
    if args.emit:
        ...
        print("OGE_6_6_OBJECT_EVIDENCE_AUDIT=PASS")
        ...
        print("READY_FOR_EXACT_OBJECT_ACCEPTANCE=" + str(int(summary["ready_for_separate_exact_object_acceptance"])))

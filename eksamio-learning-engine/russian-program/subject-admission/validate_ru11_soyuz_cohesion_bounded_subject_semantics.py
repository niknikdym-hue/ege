...
ACCEPTANCE = HERE / "RU11-SOYUZ-COHESION-BOUNDED-SUBJECT-SEMANTIC-ACCEPTANCE-v0.1.json"
...
CONTENT = PROGRAM / "production-learning-content/RU-PROG-11-SOYUZ-COHESION-WAVE-012-v0.1.json"

CANDIDATE = "candidate-005"
TAXONOMY = "cohesion_conjunction"
SEMANTIC = "ru-text-cohesion-conjunction"
CANONICAL_LABEL = "Союз как средство межфразовой связи"
EXPECTED_CHECK_IDS = {"p11-conj-v1", "p11-conj-v2", "p11-conj-v3"}
...
def main() -> int:
    ...
    if acceptance.get("status") != "CENTRAL_BRAIN_ACCEPTED_RU11_CONJUNCTION_COHESION_BOUNDED_SUBJECT_SEMANTIC":
        raise AssertionError("RU11 candidate-005 acceptance status drift")
    if acceptance.get("authority_issue") != 161:
        raise AssertionError("RU11 candidate-005 authority issue drift")
    if acceptance.get("canonical_school_registry_mutated") is not False or acceptance.get("new_parallel_registry_created") is not False:
        raise AssertionError("RU11 candidate-005 registry boundary drift")
    ...
    if sg.get("inventory_candidate_review_status") != "draft" or sg.get("inventory_candidate_status_silently_upgraded") is not False:
        raise AssertionError("RU11 candidate-005 inventory guard drift")
    if sg.get("taxonomy_backing_review_status") != "source_verified" or sg.get("skill_graph_evidence_status") != "confirmed":
        raise AssertionError("RU11 candidate-005 source status drift")
    if sg.get("skill_graph_parent") != "text_cohesion" or sg.get("skill_graph_task_numbers") != [26]:
        raise AssertionError("RU11 candidate-005 graph boundary drift")
    if len(sg.get("accepted_positive_scope") or []) != 3:
        raise AssertionError("RU11 candidate-005 positive scope drift")
    for key in (
        "full_conjunction_taxonomy_mastery_admitted",
        "punctuation_or_orthography_mastery_admitted",
        "generic_semantic_relation_mastery_admitted",
        "generic_task26_mastery_admitted",
        "conjunction_presence_alone_counts_as_inter_sentence_mastery",
    ):
        if sg.get(key) is not False:
            raise AssertionError(f"RU11 candidate-005 source boundary weakened: {key}")
    ...
    for key in (
        ...
        if policy.get(key) is not True:
            raise AssertionError(f"RU11 candidate-005 policy weakened: {key}")
    if policy.get("minimum_component_specific_independent_checks") != 2 or policy.get("new_subject_identity_namespace") != "ru-*":
        raise AssertionError("RU11 candidate-005 evidence/namespace policy drift")
    for key in (
        "ege_taxonomy_id_promoted_unchanged",
        "candidate_id_used_as_semantic_id",
        "adjacent_candidates_admitted",
        "full_conjunction_taxonomy_mastery_admitted_by_this_authority",
        "generic_semantic_relation_mastery_admitted_by_this_authority",
        "generic_task26_result_can_emit_exact_component_mastery",
        "conjunction_presence_alone_can_emit_exact_candidate_005_mastery",
        "broader_link_means_result_can_emit_exact_candidate_005_mastery",
        ...
        if policy.get(key) is not False:
            raise AssertionError(f"RU11 candidate-005 fail-closed policy drift: {key}")
    ...
    if any(
        ...
        raise AssertionError("RU11 candidate-005 exact school meaning exists; reuse required")
    ...
    if direct:
        raise AssertionError(f"RU11 broader direct conjunction evidence drift: {direct}")
    ...
    if reuse.get("broader_existing_unit_present") is not True or reuse.get("broader_existing_unit_reused_as_scope_and_explanation_evidence") is not True:
        raise AssertionError("RU11 candidate-005 reuse-first guard drift")
    if reuse.get("broader_existing_unit_is_exact_candidate_005_mastery_evidence") is not False:
        raise AssertionError("RU11 candidate-005 broader exact-evidence drift")
    if reuse.get("broader_existing_unit_direct_conjunction_independent_checks") != 0 or reuse.get("broader_existing_direct_check_ids") != []:
        raise AssertionError("RU11 candidate-005 broader direct-check drift")
    if reuse.get("minimum_component_specific_independent_checks_for_this_acceptance") != 2:
        raise AssertionError("RU11 candidate-005 minimum evidence drift")
    if reuse.get("exact_new_unit_component_specific_conjunction_checks") != 3:
        raise AssertionError("RU11 candidate-005 new-check count drift")
    if reuse.get("new_content_materialized_only_for_proven_component_verification_gap") is not True:
        raise AssertionError("RU11 candidate-005 content-gap justification drift")

    if content.get("status") != "SUBJECT_ACCEPTANCE_REQUIRED" or content.get("module_id") != "RU-PROG-11":
        raise AssertionError("RU11 candidate-005 content status/module drift")
    ...
    if guard.get("source_passages_copied") != 0 or guard.get("commercial_textbook_bytes") != 0 or guard.get("learner_examples") != "ORIGINAL_EKSAMIO":
        raise AssertionError("RU11 candidate-005 copyright guard drift")
    content_reuse = content.get("reuse_review") or {}
    if content_reuse.get("broader_existing_unit_direct_conjunction_independent_checks") != 0 or content_reuse.get("broader_existing_direct_check_ids") != []:
        raise AssertionError("RU11 candidate-005 content reuse drift")
    if content_reuse.get("broader_existing_unit_reused_as_exact_candidate_005_mastery_evidence") is not False:
        raise AssertionError("RU11 candidate-005 content false reuse")

    unit = one([r for r in content.get("units", []) if isinstance(r, dict) and r.get("proposed_semantic_id") == SEMANTIC], "RU11 candidate-005 learner unit")
    if unit.get("title_ru") != CANONICAL_LABEL:
        raise AssertionError("RU11 candidate-005 learner title drift")
    for key, minimum in (
        ...
        if not isinstance(unit.get(key), list) or len(unit[key]) < minimum:
            raise AssertionError(f"RU11 candidate-005 learner section incomplete: {key}")
    ...
    if {str(r.get("id")) for r in checks} != EXPECTED_CHECK_IDS or len(checks) != 3:
        raise AssertionError("RU11 candidate-005 exact verification ids drift")
    if any(r.get("type") != "constructed_response" for r in checks):
        raise AssertionError("RU11 candidate-005 verification must remain constructed-response")
    if any((r.get("scoring") or {}).get("max_points") != 3 or len((r.get("scoring") or {}).get("criteria") or []) != 3 for r in checks):
        raise AssertionError("RU11 candidate-005 verification scoring drift")
    criteria_text = " ".join(str(c).lower() for r in checks for c in ((r.get("scoring") or {}).get("criteria") or []))
    for needle in ("союз", "связ", "межфраз"):
        if needle not in criteria_text:
            raise AssertionError(f"RU11 candidate-005 scoring boundary missing: {needle}")
    ...
    if peis.get("semantic_ref_status") != "PROPOSED_NOT_CANONICAL" or peis.get("source_candidate_ref") != CANDIDATE or peis.get("source_candidate_review_status") != "draft":
        raise AssertionError("RU11 candidate-005 PEIS source identity drift")
    if peis.get("independent_verification_required") is not True or peis.get("exact_mastery_requires_two_or_more_component_specific_conjunction_checks") is not True:
        raise AssertionError("RU11 candidate-005 PEIS independent-evidence guard drift")
    for key in (
        "generic_task26_result_can_emit_exact_component_mastery",
        "generic_semantic_relation_result_can_emit_this_mastery",
        "broader_link_means_unit_can_emit_exact_conjunction_mastery",
    ):
        if peis.get(key) is not False:
            raise AssertionError(f"RU11 candidate-005 PEIS fail-closed drift: {key}")

    decision = one([r for r in (acceptance.get("decisions") or []) if isinstance(r, dict)], "RU11 candidate-005 decision")
    if decision.get("candidate_ref") != CANDIDATE or decision.get("source_taxonomy_id") != TAXONOMY or decision.get("accepted_semantic_id") != SEMANTIC:
        raise AssertionError("RU11 candidate-005 decision identity drift")
    if decision.get("subject_semantic_status") != "CENTRAL_BRAIN_ACCEPTED_BOUNDED_SUBJECT_SEMANTIC":
        raise AssertionError("RU11 candidate-005 decision status drift")
    if decision.get("object_binding_status") != "NOT_BOUND_TO_ANY_EXACT_ADMISSION_UNIT_OR_REQUIREMENT":
        raise AssertionError("RU11 candidate-005 object-binding drift")

    summary = acceptance.get("summary") or {}
    expected_zero = (
        "adjacent_candidates_admitted",
        "full_conjunction_taxonomy_masteries_admitted",
        "generic_semantic_relation_masteries_admitted",

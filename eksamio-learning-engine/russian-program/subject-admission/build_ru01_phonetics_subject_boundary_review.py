...
TARGET_MODULE = "RU-PROG-01"
BROAD_MEANING = "Анализировать звуковую и буквенную форму языковых единиц."
EXPECTED_IDS = {
...
def build_review() -> dict[str, Any]:
    ...
    if not isinstance(module, dict):
        raise ValueError("RU01 missing from full-subject program")
    if module.get("semantic_binding_mode") != "SOURCE_BACKED_SUBJECT_EXPANSION_REQUIRED":
        raise ValueError("RU01 is no longer an explicit source-backed subject-expansion module")
    if module.get("candidate_refs") != []:
        raise ValueError("RU01 unexpectedly acquired draft candidate refs; re-review duplicate boundary")
    if set(module.get("domains") or []) != {"phonetics", "graphics"}:
        raise ValueError("RU01 domain boundary drift")

    content = json.loads(CONTENT.read_text(encoding="utf-8"))
    if content.get("status") != "SUBJECT_ACCEPTANCE_REQUIRED" or content.get("module_id") != TARGET_MODULE:
        raise ValueError("RU01 learner content self-admitted or module drifted")
    ...
    if not any(isinstance(row, dict) and row.get("kind") == "official_program" and row.get("access") == "PUBLIC_OFFICIAL_PDF" for row in provenance):
        raise ValueError("RU01 lacks public official-program provenance")
    units = content.get("units")
    if not isinstance(units, list) or len(units) != 3:
        raise ValueError("RU01 must contain exactly three bounded learner units")
    content_ids = {str(row.get("proposed_semantic_id", "")) for row in units if isinstance(row, dict)}
    if content_ids != EXPECTED_IDS:
        raise ValueError(f"RU01 proposed semantic set drift: {sorted(content_ids)}")
    for unit in units:
        if not isinstance(unit, dict):
            raise ValueError("invalid RU01 learner unit")
        peis = unit.get("peis_evidence") or {}
        if peis.get("semantic_ref_status") != "PROPOSED_NOT_CANONICAL":
            raise ValueError(f"RU01 content self-admitted: {unit.get('proposed_semantic_id')}")
        if peis.get("independent_verification_required") is not True:
            raise ValueError(f"RU01 independent-verification guard missing: {unit.get('proposed_semantic_id')}")
        ...
        if not isinstance(explanation.get("boundaries"), list) or not explanation["boundaries"]:
            raise ValueError(f"RU01 semantic boundary missing: {unit.get('proposed_semantic_id')}")
        ...
        if not isinstance(tutor.get("allowed"), list) or not tutor["allowed"] or not isinstance(tutor.get("forbidden"), list) or not tutor["forbidden"]:
            raise ValueError(f"RU01 Tutor grounding boundary missing: {unit.get('proposed_semantic_id')}")
    ...
    if any(collisions.values()):
        raise ValueError(f"RU01 proposed id collides with current semantic inventory: {collisions}")

    namespace = runpy.run_path(str(SLICE_BUILDER))
    review_slice = namespace["build_slice"]({TARGET_MODULE})
    exact_units = [
        row for row in review_slice.get("admission_units", [])
        if row.get("normalized_meaning") == BROAD_MEANING and TARGET_MODULE in set(row.get("modules") or [])
    ]
    if not exact_units:
        raise ValueError("RU01 exact broad-domain official rows missing")
    if any(row.get("admission_status") != "SUBJECT_REVIEW_REQUIRED" for row in exact_units):
        raise ValueError("RU01 broad-domain rows unexpectedly auto-admitted")
    ...
    result: dict[str, Any] = {
        ...
        "status": "CENTRAL_BRAIN_RU01_PHONETICS_BOUNDARY_READY_ACCEPTANCE_NOT_ADMITTED",
        "authority_issue": 161,
        ...
        "duplicate_review": {
            ...
            "decision": "THREE_DISTINCT_SOURCE_BACKED_RU_SUBJECT_EXPANSION_SEMANTICS_READY_FOR_EXPLICIT_ACCEPTANCE",
        },
        "policy": {
...
def main() -> int:
    ...
    if args.emit:
        ...
        print("RU01_PHONETICS_SUBJECT_BOUNDARY_REVIEW=PASS")

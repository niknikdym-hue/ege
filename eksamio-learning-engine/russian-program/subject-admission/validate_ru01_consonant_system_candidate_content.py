#!/usr/bin/env python3
"""Fail-closed readiness gate for the source-bounded RU01 consonant-system candidate.
...
CLAUSES = ["EDSOO59-P187-4.1.2", "OGE-COD-P020-4.1.2"]
...
def main() -> int:
    ...
    require("narrower" in str(owner_rows[0].get("coverage_note", "")).lower(), "partial-owner boundary missing")
    ...
    require({row.get("official_requirement") for row in official} == {"Система согласных звуков"}, "official scope widened")
    require({row.get("document_id") for row in official} == {"EDSOO59", "OGE_COD"}, "official documents drift")
    require(any("4.1.2" in str(row.get("source_locator")) for row in official if row.get("document_id") == "EDSOO59"), "EDSOO locator drift")
    require(any("4.1.2" in str(row.get("source_locator")) for row in official if row.get("document_id") == "OGE_COD"), "OGE locator drift")
    ...
    require(identity.get("source_taxonomy_id") == "consonant_system", "taxonomy drift")
    ...
    require("согласн" in text and "букв" in text, "sound-letter boundary missing")
    require("звонк" in text and "глух" in text, "voicing system boundary missing")
    require("твёрд" in text and "мягк" in text, "hardness system boundary missing")
    require("пар" in text and "непар" in text, "paired/unpaired system boundary missing")
    ...
    require('"ru-phonetics-consonant-system"' in owner_text, "owner candidate missing")
    require('"EDSOO59-P187-4.1.2", "OGE-COD-P020-4.1.2"' in owner_text, "owner clauses drift")
    require("FULL_CONSONANT_SYSTEM_SCOPE" in owner_text, "owner boundary drift")

    print("RU01_CONSONANT_SYSTEM_CANDIDATE_CONTENT_READINESS=PASS")

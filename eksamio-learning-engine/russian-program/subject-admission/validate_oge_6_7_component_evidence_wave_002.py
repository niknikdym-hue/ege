...
PACK = ENGINE / "russian-program" / "production-learning-content" / "RU-PROG-08-OGE-6.7-COMPONENT-EVIDENCE-WAVE-002-v0.1.json"
...
EXPECTED_ITEM_IDS = {
    "school-adjective-ending-inflection-and-special-forms": ["oge67-adjpart-v1", "oge67-adjpart-v2", "oge67-adjpart-v3"],
    "school-verb-personal-ending-conjugation-base": ["oge67-verbend-v1", "oge67-verbend-v2", "oge67-verbend-v3"],
    "school-numeral-case-ending-inflection-base": ["oge67-numbase-v1", "oge67-numbase-v2", "oge67-numbase-v3"],
    "school-numerals-two-form-paradigm-40-90-100": ["oge67-num4090100-v1", "oge67-num4090100-v2", "oge67-num4090100-v3"],
    "school-numerals-both-parts-decline-50-80-200-900": ["oge67-numboth-v1", "oge67-numboth-v2", "oge67-numboth-v3"],
}
EXPECTED_CORRECT_OPTIONS = {
    "oge67-adjpart-v1": "о синем море",
    "oge67-adjpart-v2": "с раскачивающимся фонарём",
    "oge67-verbend-v1": "клеишь",
    "oge67-verbend-v2": "пишут",
    "oge67-numbase-v1": "к пяти участникам",
    "oge67-numbase-v2": "с тремя друзьями",
    "oge67-num4090100-v1": "около сорока страниц",
    "oge67-num4090100-v2": "к девяноста участникам",
    "oge67-numboth-v1": "к пятидесяти участникам",
    "oge67-numboth-v2": "с двумястами страницами",
}
EXPECTED_CONSTRUCTED_TOKENS = {
    "oge67-adjpart-v3": ["строящейся", "прочитанном", "падеж"],
    "oge67-verbend-v3": ["борешься", "видишь", "I спряж", "II спряж"],
    "oge67-numbase-v3": ["восьми", "двум", "родитель", "датель"],
    "oge67-num4090100-v3": ["сто домов", "ста рублей", "косв"],
    "oge67-numboth-v3": ["восьмидесяти", "семьюстами", "обе части"],
}
...
def validate() -> dict[str, Any]:
    ...
    if pack.get("status") != "CURRENT_LAUNCH_OGE_6_7_COMPONENT_EVIDENCE_WAVE_002_NO_OBJECT_ADMISSION":
        raise ValueError("unexpected wave 002 status")
    ...
    if pack.get("target") != {
        ...
        raise ValueError("wave 002 target drift")
    ...
    for key, expected in required_policy.items():
        if policy.get(key) != expected:
            raise ValueError(f"wave 002 evidence policy drift: {key}")
    ...
    if not set(EXPECTED_OWNERS) <= set(route_owners):
        raise ValueError("wave 002 owner escaped exact OGE 6.7 frontier")
    ...
    adj_units = [row for row in o19.get("new_units") or [] if isinstance(row, dict)]
    if len(adj_units) != 1 or adj_units[0].get("unit_id") != "school-adjective-ending-inflection-and-special-forms":
        raise ValueError("adjective ending owner source authority drift")
    if "case-ending check" not in str(adj_units[0].get("learner_policy") or ""):
        raise ValueError("adjective productive case-ending boundary drift")

    o26 = wave_c.get("O26_verb_personal_endings") or {}
    verb = o26.get("new_unit") or {}
    if verb.get("unit_id") != "school-verb-personal-ending-conjugation-base":
        raise ValueError("verb ending owner source authority drift")
    if "спряжения" not in str(verb.get("canonical_label") or ""):
        raise ValueError("verb conjugation boundary drift")
    ...
    numeral = reopen_units.get("school-numeral-case-ending-inflection-base")
    if numeral is None or "OGE-2026-orthography-6.7" not in (numeral.get("fipi_routes") or []):
        raise ValueError("numeral base OGE 6.7 binding missing")
    boundary = str(numeral.get("ownership_boundary") or "")
    for owner in (
        "school-numerals-two-form-paradigm-40-90-100",
        "school-numerals-both-parts-decline-50-80-200-900",
    ):
        if owner not in boundary:
            raise ValueError(f"special numeral owner boundary missing: {owner}")
    ...
    adj_expansion = expansions.get("school-adjective-ending-inflection-and-special-forms")
    if not adj_expansion or adj_expansion.get("semantic_scope_after") != "adjective and participle case endings":
        raise ValueError("adjective/participle ending scope expansion drift")
    ...
    if sorted(by_owner) != sorted(EXPECTED_OWNERS) or len(rows) != len(by_owner):
        raise ValueError("wave 002 owner set/uniqueness drift")
    ...
    if item_count != 15 or len(seen_ids) != 15:
        raise ValueError("wave 002 exact evidence item denominator drift")
    if pack.get("summary") != {
        ...
        raise ValueError("wave 002 summary drift")
...
def main() -> int:
    result = validate()
    print("OGE_6_7_COMPONENT_EVIDENCE_WAVE_002=PASS")

...
ACCEPTANCE = HERE / "RU11-WORD-FORMS-COHESION-BOUNDED-SUBJECT-SEMANTIC-ACCEPTANCE-v0.1.json"
...
CONTENT = PROGRAM / "production-learning-content/RU-PROG-11-WORD-FORMS-COHESION-WAVE-013-v0.1.json"

CANDIDATE = "candidate-006"
TAXONOMY = "cohesion_word_forms"
SEMANTIC = "ru-text-cohesion-word-forms"
CANONICAL_LABEL = "Формы одного слова как средство межфразовой связи"
EXPECTED_CHECK_IDS = {"p11-wf-v1", "p11-wf-v2", "p11-wf-v3"}
...
def main() -> int:
    ...
    if acceptance.get("status") != "CENTRAL_BRAIN_ACCEPTED_RU11_WORD_FORMS_COHESION_BOUNDED_SUBJECT_SEMANTIC":
        raise AssertionError("RU11 candidate-006 acceptance status drift")
    if acceptance.get("authority_issue") != 161:
        raise AssertionError("RU11 candidate-006 authority issue drift")
    if acceptance.get("canonical_school_registry_mutated") is not False or acceptance.get("new_parallel_registry_created") is not False:
        raise AssertionError("RU11 candidate-006 registry boundary drift")
    ...
    if sg.get("inventory_candidate_review_status") != "draft" or sg.get("inventory_candidate_status_silently_upgraded") is not False:
        raise AssertionError("RU11 candidate-006 inventory guard drift")
    if sg.get("taxonomy_backing_review_status") != "source_verified" or sg.get("skill_graph_evidence_status") != "confirmed":
        raise AssertionError("RU11 candidate-006 source status drift")
    if sg.get("skill_graph_parent") != "text_cohesion" or sg.get("skill_graph_task_numbers") != [26]:
        raise AssertionError("RU11 candidate-006 graph boundary drift")
    if len(sg.get("accepted_positive_scope") or []) != 3:
        raise AssertionError("RU11 candidate-006 positive scope drift")
    for key in (
        "general_morphology_mastery_admitted",
        "cognate_words_mastery_admitted",
        "lexical_repetition_mastery_admitted",
        "generic_task26_mastery_admitted",
        "same_root_alone_counts_as_word_forms_mastery",
        "unchanged_repetition_counts_as_word_forms_mastery",
    ):
        if sg.get(key) is not False:
            raise AssertionError(f"RU11 candidate-006 source boundary weakened: {key}")
    ...
    for key in (
        ...
        if policy.get(key) is not True:
            raise AssertionError(f"RU11 candidate-006 policy weakened: {key}")
    if policy.get("minimum_component_specific_independent_checks") != 2 or policy.get("new_subject_identity_namespace") != "ru-*":
        raise AssertionError("RU11 candidate-006 evidence/namespace policy drift")
    for key in (
        "ege_taxonomy_id_promoted_unchanged",
        "candidate_id_used_as_semantic_id",
        "adjacent_candidates_admitted",
        "general_morphology_mastery_admitted_by_this_authority",
        "cognate_words_mastery_admitted_by_this_authority",
        "lexical_repetition_mastery_admitted_by_this_authority",
        "generic_task26_result_can_emit_exact_component_mastery",
        "same_root_alone_can_emit_exact_candidate_006_mastery",
        "unchanged_repetition_can_emit_exact_candidate_006_mastery",
        "broader_link_means_result_can_emit_exact_candidate_006_mastery",
        ...
        if policy.get(key) is not False:
            raise AssertionError(f"RU11 candidate-006 fail-closed policy drift: {key}")
    ...
    if any(
        ...
        raise AssertionError("RU11 candidate-006 exact school meaning exists; reuse required")
    ...
    direct = [
        ...
        if "форм" in json.dumps(row, ensure_ascii=False).lower()
    ]
    if direct:
        raise AssertionError(f"RU11 broader direct word-forms evidence drift: {direct}")
    ...
    if reuse.get("broader_existing_unit_present") is not True or reuse.get("broader_existing_unit_reused_as_scope_and_explanation_evidence") is not True:
        raise AssertionError("RU11 candidate-006 reuse-first guard drift")
    if reuse.get("broader_existing_unit_is_exact_candidate_006_mastery_evidence") is not False:
        raise AssertionError("RU11 candidate-006 broader exact-evidence drift")
    if reuse.get("broader_existing_unit_direct_word_forms_independent_checks") != 0 or reuse.get("broader_existing_direct_check_ids") != []:
        raise AssertionError("RU11 candidate-006 broader direct-check drift")
    if reuse.get("minimum_component_specific_independent_checks_for_this_acceptance") != 2:
        raise AssertionError("RU11 candidate-006 minimum evidence drift")
    if reuse.get("exact_new_unit_component_specific_word_forms_checks") != 3:
        raise AssertionError("RU11 candidate-006 new-check count drift")
    if reuse.get("new_content_materialized_only_for_proven_component_verification_gap") is not True:
        raise AssertionError("RU11 candidate-006 content-gap justification drift")

    if content.get("status") != "SUBJECT_ACCEPTANCE_REQUIRED" or content.get("module_id") != "RU-PROG-11":
        raise AssertionError("RU11 candidate-006 content status/module drift")
    ...
    if guard.get("source_passages_copied") != 0 or guard.get("commercial_textbook_bytes") != 0 or guard.get("learner_examples") != "ORIGINAL_EKSAMIO":
        raise AssertionError("RU11 candidate-006 copyright guard drift")
    content_reuse = content.get("reuse_review") or {}
    if content_reuse.get("broader_existing_unit_direct_word_forms_independent_checks") != 0 or content_reuse.get("broader_existing_direct_check_ids") != []:
        raise AssertionError("RU11 candidate-006 content reuse drift")
    if content_reuse.get("broader_existing_unit_reused_as_exact_candidate_006_mastery_evidence") is not False:
        raise AssertionError("RU11 candidate-006 content false reuse")

    unit = one([r for r in content.get("units", []) if isinstance(r, dict) and r.get("proposed_semantic_id") == SEMANTIC], "RU11 candidate-006 learner unit")
    if unit.get("title_ru") != CANONICAL_LABEL:
        raise AssertionError("RU11 candidate-006 learner title drift")
    for key, minimum in (
        ...
        if not isinstance(unit.get(key), list) or len(unit[key]) < minimum:
            raise AssertionError(f"RU11 candidate-006 learner section incomplete: {key}")
    ...
    if {str(r.get("id")) for r in checks} != EXPECTED_CHECK_IDS or len(checks) != 3:
        raise AssertionError("RU11 candidate-006 exact verification ids drift")
    if any(r.get("type") != "constructed_response" for r in checks):
        raise AssertionError("RU11 candidate-006 verification must remain constructed-response")
    if any((r.get("scoring") or {}).get("max_points") != 3 or len((r.get("scoring") or {}).get("criteria") or []) != 3 for r in checks):
        raise AssertionError("RU11 candidate-006 verification scoring drift")
    criteria_text = " ".join(str(c).lower() for r in checks for c in ((r.get("scoring") or {}).get("criteria") or []))
    for needle in ("лексем", "граммат", "форм", "границ"):
        if needle not in criteria_text:
            raise AssertionError(f"RU11 candidate-006 scoring boundary missing: {needle}")
    ...
    if peis.get("semantic_ref_status") != "PROPOSED_NOT_CANONICAL" or peis.get("source_candidate_ref") != CANDIDATE or peis.get("source_candidate_review_status") != "draft":
        raise AssertionError("RU11 candidate-006 PEIS source identity drift")
    if peis.get("independent_verification_required") is not True or peis.get("exact_mastery_requires_two_or_more_component_specific_word_forms_checks") is not True:
        raise AssertionError("RU11 candidate-006 PEIS independent-evidence guard drift")
    for key in (
        "generic_task26_result_can_emit_exact_component_mastery",
        "generic_morphology_result_can_emit_this_mastery",
        "cognate_words_result_can_emit_this_mastery",
        "broader_link_means_unit_can_emit_exact_word_forms_mastery",
        "unchanged_lexical_repetition_can_emit_exact_word_forms_mastery",
    ):
        if peis.get(key) is not False:
            raise AssertionError(f"RU11 candidate-006 PEIS fail-closed drift: {key}")

    decision = one([r for r in (acceptance.get("decisions") or []) if isinstance(r, dict)], "RU11 candidate-006 decision")
    ...
    if summary.get("accepted_bounded_subject_semantics") != 1 or summary.get("accepted_ru_subject_semantics") != 1:
        raise AssertionError("RU11 candidate-006 accepted summary drift")
    if summary.get("source_backed_candidates_consumed") != 1 or summary.get("new_original_production_content_units_used") != 1:
        raise AssertionError("RU11 candidate-006 content/source summary drift")
    for key in (
        "adjacent_candidates_admitted",
        "general_morphology_masteries_admitted",
        "cognate_words_masteries_admitted",
        ...
        if summary.get(key) != 0:
            raise AssertionError(f"RU11 candidate-006 summary must remain zero: {key}")

    canonical = json.dumps(acceptance, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    print("RU11_WORD_FORMS_COHESION_BOUNDED_SUBJECT_SEMANTIC_ACCEPTANCE=PASS")
    print("SOURCE_CANDIDATE_REVIEW_STATUS=draft")
    print(f"ACCEPTED_SEMANTIC={SEMANTIC}")
    print("BROADER_EXISTING_DIRECT_WORD_FORMS_CHECKS=0")

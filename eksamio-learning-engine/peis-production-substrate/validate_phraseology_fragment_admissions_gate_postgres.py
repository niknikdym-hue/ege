...
def main() -> None:
    ...
    require(len(adapter.answer_by_id) == 199, "authority exposes exact 199 admitted phraseology items")
    require(adapter.answer_by_id["f001-alfa-i-omega"] == "альфа и омега", "first deterministic server key is pinned")
    require(adapter.answer_by_id["f009-belyy-svet"] == "белый свет", "variant-bearing admitted item primary answer is pinned")
    require("f008-belaya-vorona" not in adapter.answer_by_id, "non-exact/non-exam row remains outside admission")
    for boundary in (
    ...
    try:
        ...
        invalid_cases = [
            ...
            ("unknown-item", {**evidence_payload(request_id="phraseology-unknown-item-0001"), "item_id": "not-a-live-id"}),
            ("excluded-285-item", {**evidence_payload(request_id="phraseology-excluded-item-0001"), "item_id": "f008-belaya-vorona", "response_text": "белая ворона"}),
            ...
            ("client-variants", {**evidence_payload(request_id="phraseology-client-variants-0001"), "variants": ["на белом свете"]}),
            ("missing-component", {**evidence_payload(request_id="phraseology-missing-component-0001"), "missing_component": "омега"}),
        ...
        accepted_payload = evidence_payload(
            request_id="phraseology-accepted-0001",
            response_text="  АЛЬФА   И ОМЕГА  ",
        )
        ...
        }], "semantic target is server-selected exact phraseology action component")
        require(event["source"]["object_id"] == "f001-alfa-i-omega", "exact live phraseology item identity is preserved")
        require(event["result"]["correctness"] is True and event["result"]["score"] == 1, "server deterministically scores bounded normalized exact response")
        require(event["result"]["response_value"] == "  АЛЬФА   И ОМЕГА  ", "observed typed response is preserved")
        require(event["response_mode"] == "TYPED_TEXT", "phraseology observation remains typed text")
        ...
        require(extension["variant_answers_admitted"] is False, "variants cannot leak into exact fragment answer key")
        ...
        mutated["response_text"] = "альфа или омега"
        ...
        # Explicit live variant exists for this row, but it is absent from the exact prompt
        # and is therefore not an accepted answer for fragment identification.
        variant_payload = evidence_payload(
            request_id="phraseology-variant-is-wrong-0002",
            item_id="f009-belyy-svet",
            response_text="на белом свете",
        )
        status, _headers, variant_result = request_json(
            port,
            "POST",
            admissions_web.PHRASEOLOGY_ADMISSION_PATH,
            cookie=cookie,
        ...
        require(status == 200 and variant_result.get("status") == "ACCEPTED", "variant text remains valid observed evidence")
        variant_event = inspection.raw_event(str(variant_result["event_receipt"]["event_id"]))
        require(variant_event is not None and variant_event["result"]["correctness"] is False, "non-prompt variant is deterministically incorrect")
        require(variant_event["error_observations"][0]["semantic_id"] == SEMANTIC_ID, "incorrect exact action produces exact semantic error evidence")

        all_events = inspection.list_events(event["learner_profile_id"], "russian", effective=False)
        require(len(all_events) == 2, "only two admitted phraseology observations persisted; rejected probes mutated nothing")
        ...
        print("variant_answers_admitted=0 remaining_86_rows_admitted=0 false_exact_mastery=0 provider_calls=0")
    finally:

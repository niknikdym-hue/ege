...
HERE="$(cd "$(dirname "$0")" && pwd)"; RUNTIME="$(cd "$HERE/.." && pwd)/gated_runtime.py"
PYTHON="${EKSAMIO_PYTHON:-$(command -v python3)}"; "$PYTHON" -c "import jsonschema; import sys; sys.path.insert(0,'$(dirname "$RUNTIME")'); import runtime, registered_runtime, gated_runtime"

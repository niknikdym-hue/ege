...
with sync_playwright() as p:
    ...
    check(page.get_by_text('Предварительная автоматическая проверка',exact=False).count()>=1,'preliminary-analysis label shown')
    check(page.get_by_text('не заменяет экспертную проверку',exact=False).count()>=1,'expert-review disclaimer shown')
    ...
    check(page.get_by_text('не влияют на балл или hard cap',exact=False).count()>=1,'technical note has no scoring effect')
    for k in ['K7','K8','K9','K10']:page.select_option(f'[data-review-score="{k}"]','3')

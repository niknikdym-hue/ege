...
URL=os.environ.get('EKSAMIO_PREVIEW_URL','http://127.0.0.1:8765/ege-matematika-profil-demoversiya-2023-PREVIEW.html')
HERE=Path(__file__).resolve(); ROOT=Path(os.environ.get('EKSAMIO_PACKAGE_ROOT', str(HERE.parent.parent))).resolve()
OUT=ROOT/'tests'/'evidence'/'profile-2023-browser-evidence.json'
...
MAX={12:2,13:3,14:2,15:2,16:3,17:4,18:4}
SOL_COUNTS={12:1,13:1,14:1,15:1,16:2,17:3,18:2}
STORAGE='eksamio_ege_math_profile_demo_2023_v1_0'
...
def finalize(ev):
    spec=importlib.util.spec_from_file_location('b',ROOT/'scripts'/'build_profile_2023.py');b=importlib.util.module_from_spec(spec);spec.loader.exec_module(b)
    ...
    for fn in [f'{b.PREFIX}-INPUT-CONTRACT.json',f'{b.PREFIX}-EXAM-MAP.json',f'{b.PREFIX}-BUILD-EVIDENCE.json']:
        p=ROOT/fn;x=json.loads(p.read_text(encoding='utf-8'));x['status']='READY_FOR_TILDA';x['browser_audit']='tests/evidence/profile-2023-browser-evidence.json';p.write_text(json.dumps(x,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    rows=[]
    import csv
    with (ROOT/'AUDIT-MATRIX-2023-profile.csv').open('r',encoding='utf-8-sig',newline='') as f:rows=list(csv.DictReader(f))
    for r in rows:r['interaction_gate']='PASS_BROWSER'
    with (ROOT/'AUDIT-MATRIX-2023-profile.csv').open('w',encoding='utf-8-sig',newline='') as f:w=csv.DictWriter(f,fieldnames=rows[0].keys());w.writeheader();w.writerows(rows)
    ...
    (ROOT/f'{b.PREFIX}-PAGE-STATUS.txt').write_text(
      'PAGE_URL: /ege/matematika-profil/demoversiya/2023/\nPAGE_SLUG: ege-matematika-profil-demoversiya-2023\nEXAM: ЕГЭ\nSUBJECT: математика\nLEVEL: профильный\nSOURCE_YEAR: 2023\nPACKAGE_VERSION: 1.0\nSOURCE_GATE: PASS\nTEXT_TYPOGRAPHY_GATE: PASS\nFORMULA_GATE: PASS\nVISUAL_SOURCE_GATE: PASS — 47/47 direct lossless exact-source crops\nVISUAL_UI_GATE: PASS — all 35 conditions + extended material rendered; zoom tested\nINTERACTION_GATE: PASS — 35/35 official examples through real DOM controls\nSCORER_GATE: PASS — 28/28 short examples correct+wrong\nSTATE_RESTORE_GATE: PASS — variants, answers, mark, current state survive reload\nEXTENDED_UX_GATE: PASS — toolbar, quick transforms, own answer, criteria and self-evaluation 7/7\nTILDA_ATOMIC_GATE: PASS — '+str(len(blocks))+' T123, max '+str(mx)+' bytes < 42500; standalone script/style/node syntax PASS\nRESPONSIVE_GATE: PASS — 1280/768/390/360/320\nINDEPENDENT_AUDIT_GATE: PENDING_FINAL_CLEAN_ZIP\nFINAL_STATUS: READY_FOR_FINAL_CLEAN_ZIP_AUDIT\nREADY_FOR_TILDA: PENDING_FINAL_CLEAN_ZIP_AUDIT\nLIVE_GO: NO\n',encoding='utf-8')

def main():
    ev={'status':'PASS','checks':{},'details':{}}
    d=driver_new()
    try:
        d.get(URL);d.execute_script('localStorage.clear()');d.refresh();d.find_element(By.ID,'mp-start').click();assert len(d.find_elements(By.CSS_SELECTOR,'.mp-num'))==18
        st=js(d,'window.EKSAMIO_MATH_PROFILE_TEST.state()');assert all(1<=int(st['variants'][str(n)])<=COUNTS[n] for n in range(1,19));ev['checks']['assignment']='PASS'
        opened=short=ext=0
        for n in range(1,19):
            for v in range(1,COUNTS[n]+1):
                ...
                if n<=11:
                    ...
                    wrong='0,99' if n in (3,4) else ('999' if n==7 and v==1 else '999999');fill(inp,wrong);assert js(d,'window.EKSAMIO_MATH_PROFILE_TEST.scoreShort(arguments[0])',n)==0
                ...
                opened+=1
        assert (opened,short,ext)==(35,28,7);ev['checks']['official_examples']='35/35';ev['checks']['short_correct_wrong']='28/28'

        force(d,3,1);inp=d.find_element(By.ID,'mp-short');fill(inp,'50%');assert '%' in d.find_element(By.ID,'mp-error').text;fill(inp,'0.08');assert inp.get_attribute('value')=='0,08' and js(d,'window.EKSAMIO_MATH_PROFILE_TEST.scoreShort(3)')==1
        force(d,7,1);inp=d.find_element(By.ID,'mp-short');fill(inp,'4,0');assert 'целое число' in d.find_element(By.ID,'mp-error').text;fill(inp,'4');assert js(d,'window.EKSAMIO_MATH_PROFILE_TEST.scoreShort(7)')==1
        force(d,7,2);fill(d.find_element(By.ID,'mp-short'),'-1.75');assert js(d,'window.EKSAMIO_MATH_PROFILE_TEST.scoreShort(7)')==1
        ev['checks']['input_contracts']='PASS'

        force(d,12,1);ta=d.find_element(By.ID,'mp-long');fill(ta,'');buttons=d.find_elements(By.CSS_SELECTOR,'.mp-math-btn');next(b for b in buttons if b.get_attribute('data-sym')=='π').click();next(b for b in d.find_elements(By.CSS_SELECTOR,'.mp-math-btn') if b.get_attribute('data-sym')=='√()').click();assert ta.get_attribute('value')=='π√()';assert d.execute_script('return arguments[0].selectionStart',ta)==3
        ...
        force(d,7,1);d.find_element(By.CSS_SELECTOR,'.mp-zoom-btn').click();assert 'mp-hidden' not in d.find_element(By.ID,'mp-zoom-modal').get_attribute('class');d.find_element(By.ID,'mp-zoom-in').click();assert d.find_element(By.ID,'mp-zoom-reset').text.strip()=='125%';d.find_element(By.ID,'mp-zoom-out').click();assert d.find_element(By.ID,'mp-zoom-reset').text.strip()=='100%';d.find_element(By.ID,'mp-zoom-close').click();ev['checks']['zoom']='PASS 50-300 controls present; +/- tested'

        force(d,4,2);fill(d.find_element(By.ID,'mp-short'),'0,1');force(d,15,1);fill(d.find_element(By.ID,'mp-long'),'Сохранённое решение 15');safe_click(d,d.find_element(By.ID,'mp-mark'));d.execute_script("window.dispatchEvent(new PageTransitionEvent('pagehide'));");d.refresh();after=js(d,'window.EKSAMIO_MATH_PROFILE_TEST.state()');assert after['answers']['15']['text']=='Сохранённое решение 15' and after['answers']['4']['value']=='0,1' and bool(after['marked']['15']);ev['checks']['state_restore']='PASS'

        d.execute_script('localStorage.clear()');d.refresh();d.find_element(By.ID,'mp-start').click()
        own={}
        for n in range(1,12):force(d,n,1);fill(d.find_element(By.ID,'mp-short'),ANS[n][0])
        for n in range(12,19):force(d,n,1);own[n]=f'Полное решение задания {n}';fill(d.find_element(By.ID,'mp-long'),own[n])
        assert js(d,'window.EKSAMIO_MATH_PROFILE_TEST.autoTotal()')==11;d.find_element(By.ID,'mp-finish').click();d.switch_to.alert.accept();time.sleep(.05);assert d.find_element(By.ID,'mp-auto-score').text.strip()=='11/11'
        ...
        assert d.find_element(By.ID,'mp-self-score').text.strip()=='20/20' and d.find_element(By.ID,'mp-total-score').text.strip()=='31/31';ev['checks']['extended_results']='PASS 7/7 own answer + official per-page assets + self-score';ev['checks']['scoring']='11 auto + 20 self = 31'

...
OUT=Path(os.environ.get('EKSAMIO_EVIDENCE_OUT','tests/evidence/v1.1-selenium-regression-evidence.json'))
...
def main():
    ev={'status':'PASS','package_version':'1.1','checks':{},'details':{}}
    driver=make_driver()
    try:
        driver.get(URL)
        ...
        ev['checks']['task1_decimal_wrong_persists']='PASS: 11.2 → 11,2 → navigation/reload 11,2; scorer 0/1'

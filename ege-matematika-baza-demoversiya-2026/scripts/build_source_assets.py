...
# The band begins at the TASK_NUMBER/ИЛИ marker because a diagram can sit beside
# the first text line; it ends immediately before the example's own Ответ: line.
SPECS = [
    ("base-03-v1-temperature-chart",10,59,271,1),("base-03-v3-nickel-chart",11,56,310,1),
    ("base-07-v1-derivative-graph",14,262,523,1),("base-07-v2-torque-chart",15,57,398,1),("base-07-v3-function-graphs",16,56,389,8),
    ("base-09-v1-lake-plan",18,63,302,1),("base-09-v2-grid-plan",18,322,429,1),
    ("base-10-v1-dacha-plan",19,59,150,1),("base-10-v2-wheel",19,170,251,1),("base-10-v3-fence-plan",19,271,357,3),
    ("base-11-v1-tank",20,59,137,1),("base-11-v2-cut-prism",20,157,233,1),("base-11-v3-polyhedron",20,253,396,1),("base-11-v4-boxes",20,416,507,2),
    ("base-12-v1-triangle-median",21,59,121,1),("base-12-v2-circle-chord",21,144,210,1),("base-12-v3-right-triangle",21,232,292,1),("base-12-v4-midline",21,312,388,1),
    ("base-13-v1-cone",22,62,125,1),("base-13-v2-pyramid",22,145,216,1),("base-13-v3-cylinders",22,236,318,2),("base-13-v4-spheres",22,338,401,2),
    ("base-18-v1-number-line",25,59,247,1),("base-18-v3-number-line",26,56,295,1),("base-21-v2-rectangle-partition",28,148,231,1),
]
...
def detect_components(img, words, sx, sy, y0_pt, y1_pt):
    rgb=np.array(img);masked=mask_text(rgb,words,sx,sy);gray=cv2.cvtColor(masked,cv2.COLOR_RGB2GRAY)
    binary=np.zeros_like(gray,dtype=np.uint8);top=max(0,int(y0_pt*sy));bottom=min(img.height,int(y1_pt*sy))
    ...
    n,_,stats,_=cv2.connectedComponentsWithStats(binary,8);comps=[]
    for i in range(1,n):
...
for asset_id,page,y0_pt,y1_pt,expected_count in SPECS:
    ...
    if len(comps)<expected_count:raise RuntimeError(f'{asset_id}: need {expected_count} visual components, found {len(comps)}')
    chosen=comps[:expected_count]
    px0=min(c['x'] for c in chosen);py0=min(c['y'] for c in chosen);px1=max(c['x']+c['w'] for c in chosen);py1=max(c['y']+c['h'] for c in chosen)
    ...
    if labels:bbox=[min(bbox[0],min(w['x0'] for w in labels)),min(bbox[1],min(w['y0'] for w in labels)),max(bbox[2],max(w['x1'] for w in labels)),max(bbox[3],max(w['y1'] for w in labels))]
    pad=12;bbox=[max(18,bbox[0]-pad),max(y0_pt,bbox[1]-pad),min(coord['visual_width_pt']-18,bbox[2]+pad),min(y1_pt,bbox[3]+pad)]
    ...
    local=binary[crop_px[1]:crop_px[3],crop_px[0]:crop_px[2]]>0;edge=min(5,max(1,min(local.shape)//4))
    ...
    rec={'id':asset_id,'source_page':page,'official_segment_pt':[y0_pt,y1_pt],'expected_components':expected_count,'selected_components':chosen,'crop_pt':[round(v,2) for v in bbox],'crop_px':crop_px,'width_px':crop.width,'height_px':crop.height,'bytes':len(blob),'sha256':hashlib.sha256(blob).hexdigest(),'lossless_webp':True,'edge_nontext_ink_px':edge_ink,'included_source_words':included,'status':'PASS' if edge_ink<=12 and not prose else 'FAIL'}
    records.append(rec);preview += [f'===== {asset_id} | page {page} | crop={rec["crop_pt"]} | edgeInk={edge_ink} | {rec["status"]} =====','WORDS: '+' '.join(included),ascii_preview(crop),'']
status='PASS' if not errors and len(records)==25 else 'FAIL'
evidence={'status':status,'source':'official FIPI 2026 base mathematics PDF via canonical printed-page render','method':'marker-to-answer segment + text-masked component detection + compact diagram labels + 12pt edge padding','asset_count':len(records),'errors':errors,'records':records}

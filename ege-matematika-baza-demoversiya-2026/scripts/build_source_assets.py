...
def detect(img,words,sx,sy,y0,y1):
 masked=mask_text(np.array(img),words,sx,sy);gray=cv2.cvtColor(masked,cv2.COLOR_RGB2GRAY);binary=np.zeros_like(gray,dtype=np.uint8);top=int(y0*sy);bottom=int(y1*sy);binary[top:bottom,:]=(gray[top:bottom,:]<225).astype(np.uint8)*255;binary[:,:int(18*sx)]=0;binary[:,int((img.width/sx-18)*sx):]=0;binary=cv2.morphologyEx(binary,cv2.MORPH_CLOSE,cv2.getStructuringElement(cv2.MORPH_RECT,(3,3)),iterations=1);binary=cv2.dilate(binary,cv2.getStructuringElement(cv2.MORPH_RECT,(5,5)),iterations=1);n,_,stats,_=cv2.connectedComponentsWithStats(binary,8);c=[]
 for i in range(1,n):
 ...
 c.sort(key=lambda z:(z['area'],z['bbox_area']),reverse=True);return c,binary,top,bottom
...
for aid,page,y0,y1,count in SPECS:
 img=Image.open(PAGES/f'page-{page:02d}.webp').convert('RGB');coord=json.loads((COORD/f'page-{page:02d}.json').read_text(encoding='utf-8'));sx=img.width/coord['visual_width_pt'];sy=img.height/coord['visual_height_pt'];comps,binary,band_top,band_bottom=detect(img,coord['words'],sx,sy,y0,y1)
 if len(comps)<count:raise RuntimeError(f'{aid}: need {count} components, found {len(comps)}')
 chosen=comps[:count];px0=min(c['x']for c in chosen);py0=min(c['y']for c in chosen);px1=max(c['x']+c['w']for c in chosen);py1=max(c['y']+c['h']for c in chosen);gaps={'top':py0-band_top,'bottom':band_bottom-py1,'left':px0-int(18*sx),'right':int((coord['visual_width_pt']-18)*sx)-px1};clipped=any(v<=0 for v in gaps.values())
 if clipped:errors.append(f'{aid}: detected geometry intersects official segment boundary {gaps}')
 ...
 pad=12;bbox=[max(18,bbox[0]-pad),max(y0,bbox[1]-pad),min(coord['visual_width_pt']-18,bbox[2]+pad),min(y1,bbox[3]+pad)];cp=[int(bbox[0]*sx),int(bbox[1]*sy),int(np.ceil(bbox[2]*sx)),int(np.ceil(bbox[3]*sy))];crop=img.crop(tuple(cp));local=binary[cp[1]:cp[3],cp[0]:cp[2]]>0;e=min(5,max(1,min(local.shape)//4));edge=int(local[:e,:].sum()+local[-e:,:].sum()+local[:,:e].sum()+local[:,-e:].sum())if local.size else 999999;path=ASSETS/f'{aid}.webp';crop.save(path,'WEBP',lossless=True,method=6);blob=path.read_bytes();included=[w['text']for w in coord['words']if not(w['x1']<bbox[0]or w['x0']>bbox[2]or w['y1']<bbox[1]or w['y0']>bbox[3])];prose=[x for x in included if len(x.strip('.,:;()'))>14]
 if prose:errors.append(f'{aid}: likely prose in crop {prose[:5]}')
 status='PASS' if not clipped and not prose else'FAIL';rec={'id':aid,'source_page':page,'official_segment_pt':[y0,y1],'expected_components':count,'selected_components':chosen,'segment_boundary_gap_px':gaps,'geometry_clipped':clipped,'crop_pt':[round(v,2)for v in bbox],'crop_px':cp,'width_px':crop.width,'height_px':crop.height,'bytes':len(blob),'sha256':hashlib.sha256(blob).hexdigest(),'lossless_webp':True,'edge_nontext_ink_px_diagnostic_only':edge,'included_source_words':included,'status':status};records.append(rec);pre += [f'===== {aid} page={page} gaps={gaps} edgeDiagnostic={edge} {status} =====','WORDS: '+' '.join(included),preview(crop),'']
status='PASS' if not errors and len(records)==25 else'FAIL';ev={'status':status,'source':'official FIPI 2026 base mathematics PDF via canonical printed-page render','method':'marker-to-answer segment; completeness proven by component bounds strictly inside source segment; edge ink retained as diagnostic only because official drawings can sit 1–2 px from answer line','asset_count':len(records),'errors':errors,'records':records};(ROOT/'source-evidence'/'ASSET-CROP-EVIDENCE.json').write_text(json.dumps(ev,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');(ROOT/'source-diagnostics'/'ASSET-CROP-PREVIEWS.txt').write_text('\n'.join(pre)+'\n',encoding='utf-8');print(json.dumps({'status':status,'assets':len(records),'errors':errors},ensure_ascii=False,indent=2));raise SystemExit(0 if status=='PASS' else 1)

...
def build_assets(layout: dict, scorer: dict) -> tuple[dict[str, str], list[dict]]:
    ...
    for ext in scorer["extended_tasks"]:
        ...
        for idx, p in enumerate(ext["criteria_physical_pages"], 1):
            raw = png_bytes(page(int(p)))
            key = f"criteria-{n:02d}-{idx:02d}"
            assets[key] = base64.b64encode(raw).decode("ascii")
            manifest.append({
                "key": key,
                "kind": "official_solution_and_criteria_page",
                "task": n,
                "physical_page": int(p),
                "crop_200dpi_pixels": [0, 0, EXPECTED_PAGE_SIZE[0], EXPECTED_PAGE_SIZE[1]],
                "width": EXPECTED_PAGE_SIZE[0],
                "height": EXPECTED_PAGE_SIZE[1],
                "sha256": sha256_bytes(raw),
            })
...
def app_shell() -> str:
    return r'''<style>
#ep24{--ink:#172033;--muted:#667085;--line:#e5e7eb;--soft:#f7f8fb;--accent:#3157d5;--good:#18794e;--bad:#b42318;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;color:var(--ink);max-width:980px;margin:0 auto;padding:24px 16px 72px;box-sizing:border-box}
#ep24 *{box-sizing:border-box}#ep24 button,#ep24 input,#ep24 textarea{font:inherit}
.ep24-hero{background:linear-gradient(135deg,#f4f7ff,#fff);border:1px solid #dfe6ff;border-radius:24px;padding:28px;margin-bottom:20px}.ep24-kicker{font-size:13px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#3157d5}.ep24-hero h1{font-size:clamp(28px,5vw,46px);line-height:1.08;margin:8px 0 12px}.ep24-hero p{font-size:16px;line-height:1.55;color:var(--muted);max-width:760px}.ep24-meta{display:flex;flex-wrap:wrap;gap:8px;margin-top:16px}.ep24-chip{background:#fff;border:1px solid var(--line);border-radius:999px;padding:8px 12px;font-size:14px}
.ep24-toolbar{position:sticky;top:8px;z-index:20;background:rgba(255,255,255,.94);backdrop-filter:blur(12px);border:1px solid var(--line);border-radius:16px;padding:10px 12px;display:flex;gap:10px;align-items:center;justify-content:space-between;margin:0 0 20px;box-shadow:0 8px 24px rgba(16,24,40,.06)}.ep24-progress{font-weight:700}.ep24-timer{font-variant-numeric:tabular-nums;color:var(--muted)}
.ep24-card{border:1px solid var(--line);border-radius:20px;background:#fff;padding:18px;margin:0 0 16px;box-shadow:0 2px 10px rgba(16,24,40,.035)}.ep24-card-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}.ep24-card-head h2{font-size:19px;margin:0}.ep24-points{font-size:13px;color:var(--muted);white-space:nowrap}.ep24-source{display:block;width:100%;height:auto;border:1px solid #eceff3;border-radius:12px;background:#fff}.ep24-answer{margin-top:14px}.ep24-answer label{display:block;font-weight:650;margin-bottom:7px}.ep24-answer input,.ep24-answer textarea{width:100%;border:1px solid #cfd4dc;border-radius:12px;padding:12px 13px;background:#fff;color:var(--ink);outline:none}.ep24-answer textarea{min-height:132px;resize:vertical}.ep24-answer input:focus,.ep24-answer textarea:focus{border-color:#7b94ec;box-shadow:0 0 0 3px rgba(49,87,213,.1)}
.ep24-actions{display:flex;gap:10px;flex-wrap:wrap;margin:22px 0}.ep24-btn{appearance:none;border:0;border-radius:12px;padding:12px 17px;font-weight:700;cursor:pointer}.ep24-btn-primary{background:var(--accent);color:#fff}.ep24-btn-secondary{background:#eef2ff;color:#2548be}.ep24-btn-danger{background:#fff1f0;color:#b42318}.ep24-btn:disabled{opacity:.45;cursor:not-allowed}
.ep24-results{display:none}.ep24-results.is-open{display:block}.ep24-result-hero{border-radius:22px;padding:24px;background:#111827;color:#fff;margin-bottom:18px}.ep24-score{font-size:clamp(34px,7vw,58px);font-weight:800;line-height:1}.ep24-result-hero p{color:#d1d5db}.ep24-section{margin:28px 0 12px}.ep24-section h2{font-size:24px;margin:0 0 6px}.ep24-section p{color:var(--muted);line-height:1.5}.ep24-review{border:1px solid var(--line);border-radius:16px;padding:14px 16px;margin:10px 0}.ep24-review[data-correct="true"]{border-color:#a6d8c3;background:#f2fbf6}.ep24-review[data-correct="false"]{border-color:#f5c3bd;background:#fff8f7}.ep24-review h3{margin:0 0 8px;font-size:16px}.ep24-review p{margin:5px 0;line-height:1.4}.ep24-self{border:1px solid var(--line);border-radius:18px;padding:16px;margin:14px 0}.ep24-self-head{display:flex;justify-content:space-between;gap:12px;align-items:center}.ep24-self h3{margin:0}.ep24-score-buttons{display:flex;gap:7px;flex-wrap:wrap;margin:12px 0}.ep24-score-btn{border:1px solid #cfd4dc;background:#fff;border-radius:10px;padding:8px 12px;cursor:pointer;font-weight:700}.ep24-score-btn.is-on{background:#3157d5;border-color:#3157d5;color:#fff}.ep24-criteria details{margin-top:12px}.ep24-criteria summary{cursor:pointer;font-weight:700}.ep24-criteria img{display:block;width:100%;height:auto;border:1px solid #e5e7eb;border-radius:12px;margin-top:10px}.ep24-note{font-size:13px;color:var(--muted)}.ep24-hidden{display:none!important}.ep24-footer-note{margin-top:28px;padding:16px;border-radius:14px;background:var(--soft);color:var(--muted);font-size:13px;line-height:1.5}
@media(max-width:560px){#ep24{padding:14px 10px 56px}.ep24-hero{padding:20px 16px;border-radius:18px}.ep24-card{padding:12px;border-radius:16px}.ep24-toolbar{top:4px}.ep24-card-head,.ep24-self-head{align-items:flex-start}.ep24-source{border-radius:8px}.ep24-btn{width:100%}}
</style>
<div id="ep24">
<section class="ep24-hero"><div class="ep24-kicker">Эксамио · Физика</div><h1>Демоверсия ЕГЭ 2024</h1><p>Все формулировки, формулы, обозначения, таблицы и рисунки заданий показываются как точные растровые фрагменты официальной демоверсии ФИПИ 2024. Задания 1–20 проверяются автоматически; задания 21–26 оцениваются по официальным решениям и критериям.</p><div class="ep24-meta"><span class="ep24-chip">26 заданий</span><span class="ep24-chip">235 минут</span><span class="ep24-chip">максимум 45 баллов</span><span class="ep24-chip">официальный источник: ФИПИ 2024</span></div></section>
<div class="ep24-toolbar"><div class="ep24-progress" id="ep24-progress">Заполнено 0 из 26</div><div class="ep24-timer" id="ep24-timer">03:55:00</div></div>
<main id="ep24-tasks"></main>
<div class="ep24-actions" id="ep24-actions"><button class="ep24-btn ep24-btn-primary" id="ep24-finish" type="button">Завершить и проверить</button><button class="ep24-btn ep24-btn-secondary" id="ep24-reset" type="button">Сбросить ответы</button></div>
<section class="ep24-results" id="ep24-results">
<div class="ep24-result-hero"><div class="ep24-score" id="ep24-total">— / 45</div><p id="ep24-result-copy">Сначала оцените задания 21–26.</p></div>
<div class="ep24-section" data-section="short-review"><h2>Проверка заданий 1–20</h2><p>Баллы рассчитаны по официальным правилам демоверсии ФИПИ 2024, включая частичный балл для заданий с выбором и установлением соответствия.</p></div><div id="ep24-short-review"></div>
<div class="ep24-section" data-section="self-assessment"><h2>Самооценка заданий 21–26</h2><p>Сравните своё решение с официальным возможным решением и критериями ФИПИ, затем выберите балл. Итог появится после оценки всех шести заданий.</p></div><div id="ep24-self"></div>
</section>
<div class="ep24-footer-note">Источник содержания: официальная демоверсия ЕГЭ 2024 по физике ФИПИ. Встроенные изображения заданий и критериев сформированы непосредственно из канонического PDF без переноса содержания из других лет.</div>
</div>
<script>window.EP24_A=window.EP24_A||{};</script>
'''


def runtime_js(scorer: dict) -> str:
    short = scorer["short_tasks"]
    extended = scorer["extended_tasks"]
    short_json = html_escape_json(short)
    ext_json = html_escape_json(extended)
    return f'''<script>
(function(){{
"use strict";
...
function taskImage(n){{return "data:image/png;base64,"+EP24_A["task-"+String(n).padStart(2,"0")]}}
...
function criteriaImages(n){{return Object.keys(EP24_A).filter(function(k){{return k.indexOf("criteria-"+String(n).padStart(2,"0")+"-")===0}}).sort().map(function(k){{return '<img loading="lazy" alt="Официальное решение и критерии ФИПИ 2024 для задания '+n+'" src="data:image/png;base64,'+EP24_A[k]+'">'}}).join("")}}
...
def main() -> None:
    ...
    acceptance = {
        ...
        "build_ready_task_rendering": "exact 200-dpi raster crops from canonical FIPI 2024 PDF; pdftotext task text is not used in production rendering",
        "task_source_regions": 26,
        "criteria_source_pages": len([x for x in asset_manifest if x["kind"] == "official_solution_and_criteria_page"]),
    ...
    print(json.dumps({"archive": str(ZIP), "sha256": zip_sha, "t123_count": len(blocks), "assets": len(asset_manifest)}, ensure_ascii=False))

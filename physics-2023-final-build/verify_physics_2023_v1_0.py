...
OUT = BUILD / "out" / "ege-fizika-demoversiya-2023-v1.0-TILDA-HQ-SOURCE"
DIST = BUILD / "dist"
PREFIX = "ege-fizika-demoversiya-2023"
PREVIEW = OUT / f"{PREFIX}-PREVIEW.html"
ARCHIVE = DIST / "ege-fizika-demoversiya-2023-v1.0-TILDA-HQ-SOURCE.zip"
PORT = 8765
...
def browser_gate() -> dict:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            assert_true(page.locator("#ep23").get_attribute("data-state") == "idle", "initial idle state")
            assert_true(page.locator("#ep23-calculator").count() == 1, "calculator control")
            assert_true(page.evaluate("document.documentElement.scrollWidth <= document.documentElement.clientWidth"), "desktop overflow")
            page.locator("#ep23-start").click()
            assert_true(page.locator(".ep23-nav-btn").count() == 30, "30 navigation buttons")

            task_images = 0
            for number in range(1, 31):
                page.get_by_role("button", name=str(number), exact=True).click()
                image = page.locator(".ep23-source")
                ...
                assert_true(f"задание {number} из 30" in page.locator(".ep23-task-number").inner_text().lower(), f"task {number} identity")
                task_images += 1

            scorer_checks = {
                "task4_exact": page.evaluate("EP23_TEST_SCORE(4,'25')"),
                "task4_reverse": page.evaluate("EP23_TEST_SCORE(4,'52')"),
                "task4_missing": page.evaluate("EP23_TEST_SCORE(4,'2')"),
                "task4_extra": page.evaluate("EP23_TEST_SCORE(4,'125')"),
                "task4_wrong": page.evaluate("EP23_TEST_SCORE(4,'13')"),
                "task5_exact": page.evaluate("EP23_TEST_SCORE(5,'32')"),
                "task5_one_wrong": page.evaluate("EP23_TEST_SCORE(5,'31')"),
                "task5_extra": page.evaluate("EP23_TEST_SCORE(5,'321')"),
                "task12_comma": page.evaluate("EP23_TEST_SCORE(12,'5,25')"),
                "task12_dot": page.evaluate("EP23_TEST_SCORE(12,'5.25')"),
                "task23_reverse": page.evaluate("EP23_TEST_SCORE(23,'51')"),
            }
            expected = {
                "task4_exact": 2, "task4_reverse": 2, "task4_missing": 1, "task4_extra": 1, "task4_wrong": 0,
                "task5_exact": 2, "task5_one_wrong": 1, "task5_extra": 0,
                "task12_comma": 1, "task12_dot": 1, "task23_reverse": 1,
            }
            assert_true(scorer_checks == expected, f"scorer regression {scorer_checks}")

            page.get_by_role("button", name="1", exact=True).click()
            page.locator("#ep23-answer").fill("-8")
            page.reload(wait_until="networkidle")
            assert_true(page.locator("#ep23").get_attribute("data-state") == "running", "running restore")
            assert_true(page.locator("#ep23-answer").input_value() == "-8", "answer restore")

            page.locator("#ep23-calculator").click()
            page.locator("#ep23-calc-input").fill("sqrt(9)+2^3")
            page.locator('[data-action="equals"]').click()
            assert_true(page.locator("#ep23-calc-result").inner_text().strip() == "11", "calculator result")
            page.keyboard.press("Escape")
            assert_true(page.locator("#ep23-modal").get_attribute("data-open") == "false", "calculator closes")
            assert_true(page.evaluate("document.activeElement && document.activeElement.id === 'ep23-calculator'"), "modal focus return")

            page.get_by_role("button", name="24", exact=True).click()
            page.locator('.ep23-symbol[data-symbol="μ"]').first.click()
            assert_true("μ" in page.locator("#ep23-answer").input_value(), "symbol insertion")

            official = {
                1: "-8", 2: "2", 3: "0,875", 4: "25", 5: "32", 6: "24", 7: "1", 8: "1,5", 9: "400", 10: "24",
                11: "13", 12: "5,25", 13: "1", 14: "1", 15: "45", 16: "32", 17: "42", 18: "76", 19: "13", 20: "235",
                21: "135", 22: "3,40,2", 23: "15",
            }
            for number, answer in official.items():
                page.get_by_role("button", name=str(number), exact=True).click()
                page.locator("#ep23-answer").fill(answer)
            for number in range(24, 31):
                page.get_by_role("button", name=str(number), exact=True).click()
                page.locator("#ep23-answer").fill(f"Контрольное решение {number}")

            page.once("dialog", lambda dialog: dialog.accept())
            page.locator("#ep23-finish").click()
            page.locator('#ep23[data-state="finished"]').wait_for()
            assert_true(page.locator(".ep23-review").count() == 23, "23 short review cards")
            assert_true(page.locator(".ep23-self").count() == 7, "7 self-assessment cards")
            assert_true(page.locator(".ep23-review").evaluate_all("els => els.map(e => Number(e.dataset.task)).join(',')") == ",".join(map(str, range(1, 24))), "short review order")
            assert_true(page.locator(".ep23-self").evaluate_all("els => els.map(e => Number(e.dataset.task)).join(',')") == ",".join(map(str, range(24, 31))), "extended review order")
            ...
            for number, points in max_points.items():
                page.locator(f'.ep23-self[data-task="{number}"] select').select_option(str(points))
            assert_true(page.locator("#ep23-total-score").inner_text().strip() == "54", "54/54 total")
            ...
            sentinels = ["ep23-sentinel-2025", "ep23-sentinel-2026"]
            ...
            page.reload(wait_until="networkidle")
            assert_true(page.locator("#ep23").get_attribute("data-state") == "finished", "finished restore")
            assert_true(page.locator("#ep23-total-score").inner_text().strip() == "54", "score restore")
            ...
            page.once("dialog", lambda dialog: dialog.accept())
            page.locator("#ep23-reset-result").click()
            page.locator("#ep23-start").click()
            page.locator("#ep23-answer").fill("wrong")
            page.once("dialog", lambda dialog: dialog.accept())
            page.locator("#ep23-finish").click()
            assert_true(page.locator(".ep23-review").count() == 23, "partial finish review")
            assert_true(page.locator("#ep23-short-score").inner_text().strip() == "0", "partial finish score")
...
def package_gate() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="physics-2023-clean-") as tmp:
